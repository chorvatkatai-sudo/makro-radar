window.MAKRO_DATA = {
 "erstellt": "2026-06-24T13:19:37.130Z",
 "wochenStart": "2026-06-21",
 "events": [
  {
   "title": "1-y Loan Prime Rate",
   "country": "CNY",
   "date": "2026-06-21T21:00:00-04:00",
   "impact": "Low",
   "forecast": "3.00%",
   "previous": "3.00%",
   "actual": null
  },
  {
   "title": "5-y Loan Prime Rate",
   "country": "CNY",
   "date": "2026-06-21T21:00:00-04:00",
   "impact": "Low",
   "forecast": "3.50%",
   "previous": "3.50%",
   "actual": null
  },
  {
   "title": "Credit Card Spending y/y",
   "country": "NZD",
   "date": "2026-06-21T23:00:00-04:00",
   "impact": "Low",
   "forecast": "",
   "previous": "2.9%",
   "actual": null
  },
  {
   "title": "Foreign Direct Investment ytd/y",
   "country": "CNY",
   "date": "2026-06-22T05:06:00-04:00",
   "impact": "Low",
   "forecast": "",
   "previous": "-10.3%",
   "actual": null
  },
  {
   "title": "German Buba President Nagel Speaks",
   "country": "EUR",
   "date": "2026-06-22T07:00:00-04:00",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null
  },
  {
   "title": "CPI m/m",
   "country": "CAD",
   "date": "2026-06-22T08:30:00-04:00",
   "impact": "High",
   "forecast": "0.7%",
   "previous": "0.4%",
   "actual": "0.5%"
  },
  {
   "title": "Median CPI y/y",
   "country": "CAD",
   "date": "2026-06-22T08:30:00-04:00",
   "impact": "High",
   "forecast": "2.1%",
   "previous": "2.1%",
   "actual": "2.1%"
  },
  {
   "title": "Trimmed CPI y/y",
   "country": "CAD",
   "date": "2026-06-22T08:30:00-04:00",
   "impact": "High",
   "forecast": "2.0%",
   "previous": "2.0%",
   "actual": "2.0%"
  },
  {
   "title": "Common CPI y/y",
   "country": "CAD",
   "date": "2026-06-22T08:30:00-04:00",
   "impact": "Medium",
   "forecast": "2.5%",
   "previous": "2.5%",
   "actual": null
  },
  {
   "title": "Core CPI m/m",
   "country": "CAD",
   "date": "2026-06-22T08:30:00-04:00",
   "impact": "Low",
   "forecast": "",
   "previous": "0.2%",
   "actual": "0.5%"
  },
  {
   "title": "ECB President Lagarde Speaks",
   "country": "EUR",
   "date": "2026-06-22T09:00:00-04:00",
   "impact": "Medium",
   "forecast": "",
   "previous": "",
   "actual": null
  },
  {
   "title": "FOMC Member Waller Speaks",
   "country": "USD",
   "date": "2026-06-22T09:00:00-04:00",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null
  },
  {
   "title": "Consumer Confidence",
   "country": "EUR",
   "date": "2026-06-22T10:04:00-04:00",
   "impact": "Low",
   "forecast": "-18",
   "previous": "-19",
   "actual": null
  },
  {
   "title": "ECB President Lagarde Speaks",
   "country": "EUR",
   "date": "2026-06-22T11:25:00-04:00",
   "impact": "Medium",
   "forecast": "",
   "previous": "",
   "actual": null
  },
  {
   "title": "FOMC Member Goolsbee Speaks",
   "country": "USD",
   "date": "2026-06-22T17:30:00-04:00",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null
  },
  {
   "title": "Flash Manufacturing PMI",
   "country": "AUD",
   "date": "2026-06-22T19:00:00-04:00",
   "impact": "Low",
   "forecast": "",
   "previous": "50.2",
   "actual": null
  },
  {
   "title": "Flash Services PMI",
   "country": "AUD",
   "date": "2026-06-22T19:00:00-04:00",
   "impact": "Low",
   "forecast": "",
   "previous": "47.7",
   "actual": null
  },
  {
   "title": "Flash Manufacturing PMI",
   "country": "JPY",
   "date": "2026-06-22T20:30:00-04:00",
   "impact": "Low",
   "forecast": "54.5",
   "previous": "54.5",
   "actual": null
  },
  {
   "title": "BOJ Core CPI y/y",
   "country": "JPY",
   "date": "2026-06-23T01:00:00-04:00",
   "impact": "Low",
   "forecast": "1.5%",
   "previous": "1.4%",
   "actual": null
  },
  {
   "title": "French Flash Manufacturing PMI",
   "country": "EUR",
   "date": "2026-06-23T03:15:00-04:00",
   "impact": "Medium",
   "forecast": "50.2",
   "previous": "48.9",
   "actual": null
  },
  {
   "title": "French Flash Services PMI",
   "country": "EUR",
   "date": "2026-06-23T03:15:00-04:00",
   "impact": "Medium",
   "forecast": "45.9",
   "previous": "42.9",
   "actual": null
  },
  {
   "title": "German Flash Manufacturing PMI",
   "country": "EUR",
   "date": "2026-06-23T03:30:00-04:00",
   "impact": "Medium",
   "forecast": "50.3",
   "previous": "49.9",
   "actual": null
  },
  {
   "title": "German Flash Services PMI",
   "country": "EUR",
   "date": "2026-06-23T03:30:00-04:00",
   "impact": "Medium",
   "forecast": "49.0",
   "previous": "47.8",
   "actual": null
  },
  {
   "title": "Flash Manufacturing PMI",
   "country": "EUR",
   "date": "2026-06-23T04:00:00-04:00",
   "impact": "Low",
   "forecast": "51.6",
   "previous": "51.4",
   "actual": null
  },
  {
   "title": "Flash Services PMI",
   "country": "EUR",
   "date": "2026-06-23T04:00:00-04:00",
   "impact": "Low",
   "forecast": "48.6",
   "previous": "46.4",
   "actual": null
  },
  {
   "title": "Flash Manufacturing PMI",
   "country": "GBP",
   "date": "2026-06-23T04:30:00-04:00",
   "impact": "Medium",
   "forecast": "53.5",
   "previous": "53.7",
   "actual": null
  },
  {
   "title": "Flash Services PMI",
   "country": "GBP",
   "date": "2026-06-23T04:30:00-04:00",
   "impact": "Medium",
   "forecast": "50.1",
   "previous": "47.9",
   "actual": null
  },
  {
   "title": "MPC Member Breeden Speaks",
   "country": "GBP",
   "date": "2026-06-23T04:55:00-04:00",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null
  },
  {
   "title": "CBI Industrial Order Expectations",
   "country": "GBP",
   "date": "2026-06-23T06:00:00-04:00",
   "impact": "Low",
   "forecast": "-33",
   "previous": "-41",
   "actual": null
  },
  {
   "title": "ADP Weekly Employment Change",
   "country": "USD",
   "date": "2026-06-23T08:15:00-04:00",
   "impact": "Low",
   "forecast": "",
   "previous": "25.5K",
   "actual": null
  },
  {
   "title": "MPC Member Taylor Speaks",
   "country": "GBP",
   "date": "2026-06-23T09:15:00-04:00",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null
  },
  {
   "title": "BOC Gov Macklem Speaks",
   "country": "CAD",
   "date": "2026-06-23T09:25:00-04:00",
   "impact": "Medium",
   "forecast": "",
   "previous": "",
   "actual": null
  },
  {
   "title": "Flash Manufacturing PMI",
   "country": "USD",
   "date": "2026-06-23T09:45:00-04:00",
   "impact": "Medium",
   "forecast": "54.6",
   "previous": "55.3",
   "actual": null
  },
  {
   "title": "Flash Services PMI",
   "country": "USD",
   "date": "2026-06-23T09:45:00-04:00",
   "impact": "Medium",
   "forecast": "51.1",
   "previous": "50.9",
   "actual": null
  },
  {
   "title": "Richmond Manufacturing Index",
   "country": "USD",
   "date": "2026-06-23T10:00:00-04:00",
   "impact": "Low",
   "forecast": "8",
   "previous": "13",
   "actual": null
  },
  {
   "title": "MPC Member Dhingra Speaks",
   "country": "GBP",
   "date": "2026-06-23T13:30:00-04:00",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null
  },
  {
   "title": "President Trump Speaks",
   "country": "USD",
   "date": "2026-06-23T14:05:00-04:00",
   "impact": "Medium",
   "forecast": "",
   "previous": "",
   "actual": null
  },
  {
   "title": "API Weekly Statistical Bulletin",
   "country": "USD",
   "date": "2026-06-23T16:30:00-04:00",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null
  },
  {
   "title": "BOJ Summary of Opinions",
   "country": "JPY",
   "date": "2026-06-23T19:50:00-04:00",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null
  },
  {
   "title": "SPPI y/y",
   "country": "JPY",
   "date": "2026-06-23T19:50:00-04:00",
   "impact": "Low",
   "forecast": "3.3%",
   "previous": "3.0%",
   "actual": null
  },
  {
   "title": "CPI m/m",
   "country": "AUD",
   "date": "2026-06-23T21:30:00-04:00",
   "impact": "High",
   "forecast": "-0.4%",
   "previous": "0.4%",
   "actual": "-0.7%"
  },
  {
   "title": "CPI y/y",
   "country": "AUD",
   "date": "2026-06-23T21:30:00-04:00",
   "impact": "High",
   "forecast": "4.3%",
   "previous": "4.2%",
   "actual": "4.0%"
  },
  {
   "title": "Trimmed Mean CPI m/m",
   "country": "AUD",
   "date": "2026-06-23T21:30:00-04:00",
   "impact": "High",
   "forecast": "0.3%",
   "previous": "0.3%",
   "actual": "0.5%"
  },
  {
   "title": "RBA Deputy Gov Hauser Speaks",
   "country": "AUD",
   "date": "2026-06-24T02:30:00-04:00",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null
  },
  {
   "title": "Gov Board Member Martin Speaks",
   "country": "CHF",
   "date": "2026-06-24T03:00:00-04:00",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null
  },
  {
   "title": "UBS Economic Expectations",
   "country": "CHF",
   "date": "2026-06-24T04:00:00-04:00",
   "impact": "Low",
   "forecast": "",
   "previous": "-11.1",
   "actual": null
  },
  {
   "title": "German ifo Business Climate",
   "country": "EUR",
   "date": "2026-06-24T04:00:00-04:00",
   "impact": "Low",
   "forecast": "85.6",
   "previous": "84.9",
   "actual": null
  },
  {
   "title": "German Buba President Nagel Speaks",
   "country": "EUR",
   "date": "2026-06-24T05:00:00-04:00",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null
  },
  {
   "title": "German 10-y Bond Auction",
   "country": "EUR",
   "date": "2026-06-24T05:40:00-04:00",
   "impact": "Low",
   "forecast": "",
   "previous": "3.06|1.7",
   "actual": null
  },
  {
   "title": "Gov Council Member Rogers Speaks",
   "country": "CAD",
   "date": "2026-06-24T07:15:00-04:00",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null
  },
  {
   "title": "MPC Member Breeden Speaks",
   "country": "GBP",
   "date": "2026-06-24T07:15:00-04:00",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null
  },
  {
   "title": "Current Account",
   "country": "USD",
   "date": "2026-06-24T08:30:00-04:00",
   "impact": "Low",
   "forecast": "-212B",
   "previous": "-191B",
   "actual": null
  },
  {
   "title": "Belgian NBB Business Climate",
   "country": "EUR",
   "date": "2026-06-24T08:38:00-04:00",
   "impact": "Low",
   "forecast": "-12.0",
   "previous": "-13.3",
   "actual": null
  },
  {
   "title": "SNB Quarterly Bulletin",
   "country": "CHF",
   "date": "2026-06-24T09:00:00-04:00",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null
  },
  {
   "title": "New Home Sales",
   "country": "USD",
   "date": "2026-06-24T10:00:00-04:00",
   "impact": "Low",
   "forecast": "638K",
   "previous": "622K",
   "actual": null
  },
  {
   "title": "Crude Oil Inventories",
   "country": "USD",
   "date": "2026-06-24T10:30:00-04:00",
   "impact": "Low",
   "forecast": "-3.9M",
   "previous": "-8.3M",
   "actual": null
  },
  {
   "title": "MPC Member Dhingra Speaks",
   "country": "GBP",
   "date": "2026-06-24T11:00:00-04:00",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null
  },
  {
   "title": "MPC Member Pill Speaks",
   "country": "GBP",
   "date": "2026-06-24T12:30:00-04:00",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null
  },
  {
   "title": "BOC Summary of Deliberations",
   "country": "CAD",
   "date": "2026-06-24T13:30:00-04:00",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null
  },
  {
   "title": "Bank Stress Test Results",
   "country": "USD",
   "date": "2026-06-24T16:00:00-04:00",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null
  },
  {
   "title": "Employment Change",
   "country": "AUD",
   "date": "2026-06-24T21:30:00-04:00",
   "impact": "High",
   "forecast": "31.2K",
   "previous": "-18.6K",
   "actual": null
  },
  {
   "title": "Unemployment Rate",
   "country": "AUD",
   "date": "2026-06-24T21:30:00-04:00",
   "impact": "High",
   "forecast": "4.4%",
   "previous": "4.5%",
   "actual": null
  },
  {
   "title": "Household Spending m/m",
   "country": "AUD",
   "date": "2026-06-24T21:30:00-04:00",
   "impact": "Low",
   "forecast": "0.5%",
   "previous": "-1.1%",
   "actual": null
  },
  {
   "title": "NAB Quarterly Business Confidence",
   "country": "AUD",
   "date": "2026-06-24T21:35:00-04:00",
   "impact": "Low",
   "forecast": "",
   "previous": "-4",
   "actual": null
  },
  {
   "title": "German GfK Consumer Climate",
   "country": "EUR",
   "date": "2026-06-25T02:00:00-04:00",
   "impact": "Low",
   "forecast": "-27.8",
   "previous": "-29.8",
   "actual": null
  },
  {
   "title": "ECB Economic Bulletin",
   "country": "EUR",
   "date": "2026-06-25T04:00:00-04:00",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null
  },
  {
   "title": "CBI Realized Sales",
   "country": "GBP",
   "date": "2026-06-25T06:00:00-04:00",
   "impact": "Low",
   "forecast": "-41",
   "previous": "-46",
   "actual": null
  },
  {
   "title": "Core PCE Price Index m/m",
   "country": "USD",
   "date": "2026-06-25T08:30:00-04:00",
   "impact": "High",
   "forecast": "0.3%",
   "previous": "0.2%",
   "actual": null
  },
  {
   "title": "Final GDP q/q",
   "country": "USD",
   "date": "2026-06-25T08:30:00-04:00",
   "impact": "High",
   "forecast": "1.6%",
   "previous": "1.6%",
   "actual": null
  },
  {
   "title": "Final GDP Price Index q/q",
   "country": "USD",
   "date": "2026-06-25T08:30:00-04:00",
   "impact": "Medium",
   "forecast": "3.5%",
   "previous": "3.5%",
   "actual": null
  },
  {
   "title": "Unemployment Claims",
   "country": "USD",
   "date": "2026-06-25T08:30:00-04:00",
   "impact": "Medium",
   "forecast": "225K",
   "previous": "226K",
   "actual": null
  },
  {
   "title": "Core Durable Goods Orders m/m",
   "country": "USD",
   "date": "2026-06-25T08:30:00-04:00",
   "impact": "Low",
   "forecast": "0.5%",
   "previous": "1.1%",
   "actual": null
  },
  {
   "title": "Durable Goods Orders m/m",
   "country": "USD",
   "date": "2026-06-25T08:30:00-04:00",
   "impact": "Low",
   "forecast": "-5.0%",
   "previous": "7.9%",
   "actual": null
  },
  {
   "title": "Personal Income m/m",
   "country": "USD",
   "date": "2026-06-25T08:30:00-04:00",
   "impact": "Low",
   "forecast": "0.4%",
   "previous": "0.0%",
   "actual": null
  },
  {
   "title": "Personal Spending m/m",
   "country": "USD",
   "date": "2026-06-25T08:30:00-04:00",
   "impact": "Low",
   "forecast": "0.6%",
   "previous": "0.5%",
   "actual": null
  },
  {
   "title": "CB Leading Index m/m",
   "country": "CNY",
   "date": "2026-06-25T09:00:00-04:00",
   "impact": "Low",
   "forecast": "",
   "previous": "-0.8%",
   "actual": null
  },
  {
   "title": "Natural Gas Storage",
   "country": "USD",
   "date": "2026-06-25T10:30:00-04:00",
   "impact": "Low",
   "forecast": "67B",
   "previous": "73B",
   "actual": null
  },
  {
   "title": "FOMC Member Williams Speaks",
   "country": "USD",
   "date": "2026-06-25T15:40:00-04:00",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null
  },
  {
   "title": "FOMC Member Goolsbee Speaks",
   "country": "USD",
   "date": "2026-06-25T18:30:00-04:00",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null
  },
  {
   "title": "Tokyo Core CPI y/y",
   "country": "JPY",
   "date": "2026-06-25T19:30:00-04:00",
   "impact": "Medium",
   "forecast": "1.6%",
   "previous": "1.3%",
   "actual": null
  },
  {
   "title": "German Buba President Nagel Speaks",
   "country": "EUR",
   "date": "2026-06-26T04:30:00-04:00",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null
  },
  {
   "title": "Italian 10-y Bond Auction",
   "country": "EUR",
   "date": "2026-06-26T05:33:00-04:00",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null
  },
  {
   "title": "Goods Trade Balance",
   "country": "USD",
   "date": "2026-06-26T08:30:00-04:00",
   "impact": "Low",
   "forecast": "-85.0B",
   "previous": "-82.4B",
   "actual": null
  },
  {
   "title": "Prelim Wholesale Inventories m/m",
   "country": "USD",
   "date": "2026-06-26T08:30:00-04:00",
   "impact": "Low",
   "forecast": "0.3%",
   "previous": "0.5%",
   "actual": null
  },
  {
   "title": "Revised UoM Consumer Sentiment",
   "country": "USD",
   "date": "2026-06-26T10:00:00-04:00",
   "impact": "Medium",
   "forecast": "50.0",
   "previous": "48.9",
   "actual": null
  },
  {
   "title": "Revised UoM Inflation Expectations",
   "country": "USD",
   "date": "2026-06-26T10:00:00-04:00",
   "impact": "Medium",
   "forecast": "",
   "previous": "4.6%",
   "actual": null
  },
  {
   "title": "FOMC Member Williams Speaks",
   "country": "USD",
   "date": "2026-06-26T10:30:00-04:00",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null
  },
  {
   "title": "FOMC Member Kashkari Speaks",
   "country": "USD",
   "date": "2026-06-26T11:30:00-04:00",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null
  }
 ],
 "naechsteWoche": [
  {
   "date": "2026-06-28T12:15:00.000Z",
   "country": "AUD",
   "title": "RBA Gov Bullock Speech",
   "impact": "Medium",
   "forecast": "",
   "previous": "",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-28T16:35:00.000Z",
   "country": "USD",
   "title": "Fed Barkin Speech",
   "impact": "Medium",
   "forecast": "",
   "previous": "",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-28T23:50:00.000Z",
   "country": "JPY",
   "title": "Retail Sales YoY",
   "impact": "Medium",
   "forecast": "",
   "previous": "2.1%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-28T23:50:00.000Z",
   "country": "JPY",
   "title": "Retail Sales MoM",
   "impact": "Low",
   "forecast": "",
   "previous": "1.3%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-29T00:00:00.000Z",
   "country": "EUR",
   "title": "ECB Forum on Central Banking",
   "impact": "Medium",
   "forecast": "",
   "previous": "",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-29T05:00:00.000Z",
   "country": "JPY",
   "title": "Construction Orders YoY",
   "impact": "Low",
   "forecast": "",
   "previous": "-32.3%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-29T05:00:00.000Z",
   "country": "JPY",
   "title": "Housing Starts YoY",
   "impact": "Medium",
   "forecast": "",
   "previous": "11.4",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-29T08:00:00.000Z",
   "country": "EUR",
   "title": "M3 Money Supply YoY",
   "impact": "Low",
   "forecast": "",
   "previous": "2.7%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-29T08:00:00.000Z",
   "country": "EUR",
   "title": "Loans to Companies YoY",
   "impact": "Low",
   "forecast": "",
   "previous": "3.4%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-29T08:00:00.000Z",
   "country": "EUR",
   "title": "Loans to Households YoY",
   "impact": "Low",
   "forecast": "",
   "previous": "3%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-29T08:30:00.000Z",
   "country": "GBP",
   "title": "Net Lending to Individuals MoM",
   "impact": "Low",
   "forecast": "",
   "previous": "6.2",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-29T08:30:00.000Z",
   "country": "GBP",
   "title": "BoE Consumer Credit",
   "impact": "Medium",
   "forecast": "",
   "previous": "1.859",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-29T08:30:00.000Z",
   "country": "GBP",
   "title": "M4 Money Supply MoM",
   "impact": "Low",
   "forecast": "",
   "previous": "0.2",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-29T08:30:00.000Z",
   "country": "GBP",
   "title": "Mortgage Lending",
   "impact": "Medium",
   "forecast": "",
   "previous": "4.37",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-29T08:30:00.000Z",
   "country": "GBP",
   "title": "Mortgage Approvals",
   "impact": "Medium",
   "forecast": "",
   "previous": "65.94",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-29T09:00:00.000Z",
   "country": "EUR",
   "title": "Industrial Sentiment",
   "impact": "Low",
   "forecast": "",
   "previous": "-8",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-29T09:00:00.000Z",
   "country": "EUR",
   "title": "Services Sentiment",
   "impact": "Low",
   "forecast": "",
   "previous": "2.2",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-29T09:00:00.000Z",
   "country": "EUR",
   "title": "Consumer Inflation Expectations",
   "impact": "Low",
   "forecast": "",
   "previous": "40.5",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-29T09:00:00.000Z",
   "country": "EUR",
   "title": "Consumer Confidence Final",
   "impact": "Low",
   "forecast": "-17.7",
   "previous": "-19",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-29T09:00:00.000Z",
   "country": "EUR",
   "title": "Economic Sentiment",
   "impact": "Medium",
   "forecast": "",
   "previous": "93.5",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-29T09:00:00.000Z",
   "country": "EUR",
   "title": "Selling Price Expectations",
   "impact": "Low",
   "forecast": "",
   "previous": "27.4",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-29T11:30:00.000Z",
   "country": "AUD",
   "title": "RBA Kent Speech",
   "impact": "Medium",
   "forecast": "",
   "previous": "",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-29T14:30:00.000Z",
   "country": "USD",
   "title": "Dallas Fed Manufacturing Index",
   "impact": "Medium",
   "forecast": "",
   "previous": "0.4",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-29T15:30:00.000Z",
   "country": "USD",
   "title": "6-Month Bill Auction",
   "impact": "Low",
   "forecast": "",
   "previous": "3.84%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-29T15:30:00.000Z",
   "country": "USD",
   "title": "3-Month Bill Auction",
   "impact": "Low",
   "forecast": "",
   "previous": "3.695%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-29T16:30:00.000Z",
   "country": "EUR",
   "title": "ECB President Lagarde Speech",
   "impact": "Medium",
   "forecast": "",
   "previous": "",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-29T23:01:00.000Z",
   "country": "GBP",
   "title": "BRC Shop Price Inflation",
   "impact": "Low",
   "forecast": "",
   "previous": "1.2",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-29T23:30:00.000Z",
   "country": "JPY",
   "title": "Unemployment Rate",
   "impact": "Medium",
   "forecast": "",
   "previous": "2.5",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-29T23:30:00.000Z",
   "country": "JPY",
   "title": "Jobs/applications ratio",
   "impact": "Low",
   "forecast": "",
   "previous": "1.18",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-29T23:50:00.000Z",
   "country": "JPY",
   "title": "Industrial Production MoM Prel",
   "impact": "Medium",
   "forecast": "",
   "previous": "0.5%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-29T23:50:00.000Z",
   "country": "JPY",
   "title": "Industrial Production YoY Prel",
   "impact": "Low",
   "forecast": "",
   "previous": "2%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T00:00:00.000Z",
   "country": "EUR",
   "title": "ECB Forum on Central Banking",
   "impact": "Medium",
   "forecast": "",
   "previous": "",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T01:00:00.000Z",
   "country": "NZD",
   "title": "ANZ Business Confidence",
   "impact": "Medium",
   "forecast": "",
   "previous": "10",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T01:30:00.000Z",
   "country": "AUD",
   "title": "RBA Meeting Minutes",
   "impact": "High",
   "forecast": "",
   "previous": "",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T01:30:00.000Z",
   "country": "CNY",
   "title": "NBS Manufacturing PMI",
   "impact": "High",
   "forecast": "",
   "previous": "50",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T01:30:00.000Z",
   "country": "AUD",
   "title": "Housing Credit MoM",
   "impact": "Low",
   "forecast": "",
   "previous": "0.6%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T01:30:00.000Z",
   "country": "AUD",
   "title": "Private Sector Credit YoY",
   "impact": "Low",
   "forecast": "",
   "previous": "8%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T01:30:00.000Z",
   "country": "CNY",
   "title": "NBS Non Manufacturing PMI",
   "impact": "Medium",
   "forecast": "",
   "previous": "50.1",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T01:30:00.000Z",
   "country": "CNY",
   "title": "NBS General PMI",
   "impact": "Low",
   "forecast": "",
   "previous": "50.5",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T01:30:00.000Z",
   "country": "AUD",
   "title": "Private Sector Credit MoM",
   "impact": "Low",
   "forecast": "",
   "previous": "0.7%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T02:35:00.000Z",
   "country": "NZD",
   "title": "1-Year Bill Auction",
   "impact": "Low",
   "forecast": "",
   "previous": "3.0275%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T02:35:00.000Z",
   "country": "NZD",
   "title": "3-Month Bill Auction",
   "impact": "Low",
   "forecast": "",
   "previous": "2.5568%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T02:35:00.000Z",
   "country": "NZD",
   "title": "6-Month Bill Auction",
   "impact": "Low",
   "forecast": "",
   "previous": "2.7455%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T03:35:00.000Z",
   "country": "JPY",
   "title": "2-Year JGB Auction",
   "impact": "Low",
   "forecast": "",
   "previous": "1.369%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T06:00:00.000Z",
   "country": "GBP",
   "title": "Nationwide Housing Prices MoM",
   "impact": "Medium",
   "forecast": "",
   "previous": "-0.6",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T06:00:00.000Z",
   "country": "GBP",
   "title": "Nationwide Housing Prices YoY",
   "impact": "Medium",
   "forecast": "",
   "previous": "1.7",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T06:00:00.000Z",
   "country": "GBP",
   "title": "Current Account",
   "impact": "Medium",
   "forecast": "",
   "previous": "-18.4",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T06:00:00.000Z",
   "country": "GBP",
   "title": "GDP Growth Rate YoY Final",
   "impact": "Low",
   "forecast": "1.1%",
   "previous": "1%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T06:00:00.000Z",
   "country": "GBP",
   "title": "Business Investment QoQ Final",
   "impact": "Low",
   "forecast": "0.7%",
   "previous": "-2.9%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T06:00:00.000Z",
   "country": "GBP",
   "title": "GDP Growth Rate QoQ Final",
   "impact": "Low",
   "forecast": "0.6%",
   "previous": "0.2%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T06:00:00.000Z",
   "country": "GBP",
   "title": "Business Investment YoY Final",
   "impact": "Low",
   "forecast": "-1.8%",
   "previous": "1.8%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T07:00:00.000Z",
   "country": "CHF",
   "title": "KOF Leading Indicators",
   "impact": "Medium",
   "forecast": "",
   "previous": "98",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T11:30:00.000Z",
   "country": "EUR",
   "title": "ECB Lane Speech",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T12:30:00.000Z",
   "country": "CAD",
   "title": "GDP MoM Prel",
   "impact": "Medium",
   "forecast": "",
   "previous": "0.4%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T12:30:00.000Z",
   "country": "CAD",
   "title": "GDP MoM",
   "impact": "Medium",
   "forecast": "0.4%",
   "previous": "-0.1%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T12:55:00.000Z",
   "country": "USD",
   "title": "Redbook YoY",
   "impact": "Low",
   "forecast": "",
   "previous": "10",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T13:00:00.000Z",
   "country": "USD",
   "title": "House Price Index YoY",
   "impact": "Low",
   "forecast": "",
   "previous": "1.7%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T13:00:00.000Z",
   "country": "USD",
   "title": "House Price Index",
   "impact": "Low",
   "forecast": "",
   "previous": "441.5",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T13:00:00.000Z",
   "country": "USD",
   "title": "S&P/Case-Shiller Home Price YoY",
   "impact": "Medium",
   "forecast": "",
   "previous": "0.8%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T13:00:00.000Z",
   "country": "USD",
   "title": "S&P/Case-Shiller Home Price MoM",
   "impact": "Low",
   "forecast": "",
   "previous": "1%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T13:00:00.000Z",
   "country": "USD",
   "title": "House Price Index MoM",
   "impact": "Low",
   "forecast": "",
   "previous": "0.1%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T13:45:00.000Z",
   "country": "USD",
   "title": "Chicago PMI",
   "impact": "Medium",
   "forecast": "",
   "previous": "62.7",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T14:00:00.000Z",
   "country": "USD",
   "title": "JOLTs Job Quits",
   "impact": "Low",
   "forecast": "",
   "previous": "2.977",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T14:00:00.000Z",
   "country": "USD",
   "title": "JOLTs Job Openings",
   "impact": "High",
   "forecast": "",
   "previous": "7.618",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T14:00:00.000Z",
   "country": "USD",
   "title": "CB Consumer Confidence",
   "impact": "Medium",
   "forecast": "",
   "previous": "93.1",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T14:01:00.000Z",
   "country": "AUD",
   "title": "Cotality Dwelling Prices MoM",
   "impact": "Low",
   "forecast": "",
   "previous": "0%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T14:30:00.000Z",
   "country": "USD",
   "title": "Dallas Fed Services Index",
   "impact": "Low",
   "forecast": "",
   "previous": "-7.7",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T14:30:00.000Z",
   "country": "USD",
   "title": "Dallas Fed Services Revenues Index",
   "impact": "Low",
   "forecast": "",
   "previous": "5",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T15:30:00.000Z",
   "country": "USD",
   "title": "6-Week Bill Auction",
   "impact": "Low",
   "forecast": "",
   "previous": "3.62%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T16:00:00.000Z",
   "country": "USD",
   "title": "Quarterly Grain Stocks - Soy",
   "impact": "Low",
   "forecast": "",
   "previous": "2.1",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T16:00:00.000Z",
   "country": "USD",
   "title": "Quarterly Grain Stocks - Corn",
   "impact": "Low",
   "forecast": "",
   "previous": "9.02",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T16:00:00.000Z",
   "country": "USD",
   "title": "Quarterly Grain Stocks - Wheat",
   "impact": "Low",
   "forecast": "",
   "previous": "1.3",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T20:30:00.000Z",
   "country": "USD",
   "title": "API Crude Oil Stock Change",
   "impact": "Medium",
   "forecast": "",
   "previous": "-0.765",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T23:00:00.000Z",
   "country": "AUD",
   "title": "S&P Global Manufacturing PMI Final",
   "impact": "Low",
   "forecast": "",
   "previous": "50.7",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T23:00:00.000Z",
   "country": "AUD",
   "title": "Ai Group Manufacturing Index",
   "impact": "Low",
   "forecast": "",
   "previous": "-22.4",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T23:00:00.000Z",
   "country": "AUD",
   "title": "Ai Group Industry Index",
   "impact": "Medium",
   "forecast": "",
   "previous": "-26.5",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T23:00:00.000Z",
   "country": "AUD",
   "title": "Ai Group Construction Index",
   "impact": "Low",
   "forecast": "",
   "previous": "-9.9",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T23:50:00.000Z",
   "country": "JPY",
   "title": "Tankan Large Manufacturers Index",
   "impact": "High",
   "forecast": "",
   "previous": "17",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T23:50:00.000Z",
   "country": "JPY",
   "title": "Tankan Non-Manufacturing Outlook",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T23:50:00.000Z",
   "country": "JPY",
   "title": "Tankan Large Non-Manufacturing Index",
   "impact": "Low",
   "forecast": "",
   "previous": "36",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T23:50:00.000Z",
   "country": "JPY",
   "title": "Tankan Large All Industry Capex",
   "impact": "Low",
   "forecast": "",
   "previous": "3.3",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T23:50:00.000Z",
   "country": "JPY",
   "title": "Tankan Large Manufacturing Outlook",
   "impact": "Low",
   "forecast": "",
   "previous": "14",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-06-30T23:50:00.000Z",
   "country": "JPY",
   "title": "Tankan Small Manufacturers Index",
   "impact": "Low",
   "forecast": "",
   "previous": "7",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T00:00:00.000Z",
   "country": "CAD",
   "title": "Canada Day",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T00:00:00.000Z",
   "country": "EUR",
   "title": "ECB Forum on Central Banking",
   "impact": "Medium",
   "forecast": "",
   "previous": "",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T00:30:00.000Z",
   "country": "JPY",
   "title": "S&P Global Manufacturing PMI Final",
   "impact": "Low",
   "forecast": "54.9",
   "previous": "54.5",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T01:30:00.000Z",
   "country": "AUD",
   "title": "Building Permits MoM Prel",
   "impact": "Medium",
   "forecast": "",
   "previous": "-3.4%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T01:30:00.000Z",
   "country": "AUD",
   "title": "Building Permits YoY Prel",
   "impact": "Low",
   "forecast": "",
   "previous": "10.2%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T01:30:00.000Z",
   "country": "AUD",
   "title": "Private House Approvals MoM Prel",
   "impact": "Low",
   "forecast": "",
   "previous": "-1%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T01:45:00.000Z",
   "country": "CNY",
   "title": "RatingDog Manufacturing PMI",
   "impact": "High",
   "forecast": "",
   "previous": "51.8",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T05:00:00.000Z",
   "country": "JPY",
   "title": "Consumer Confidence",
   "impact": "High",
   "forecast": "",
   "previous": "33.6",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T06:30:00.000Z",
   "country": "AUD",
   "title": "Commodity Prices YoY",
   "impact": "Low",
   "forecast": "",
   "previous": "16.8",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T06:30:00.000Z",
   "country": "CHF",
   "title": "Retail Sales YoY",
   "impact": "Medium",
   "forecast": "",
   "previous": "1.6",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T06:30:00.000Z",
   "country": "CHF",
   "title": "Retail Sales MoM",
   "impact": "Low",
   "forecast": "",
   "previous": "0.1",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T07:30:00.000Z",
   "country": "CHF",
   "title": "procure.ch Manufacturing PMI",
   "impact": "Medium",
   "forecast": "",
   "previous": "57.3",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T08:00:00.000Z",
   "country": "EUR",
   "title": "S&P Global Manufacturing PMI Final",
   "impact": "Low",
   "forecast": "51.3",
   "previous": "51.6",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T08:30:00.000Z",
   "country": "GBP",
   "title": "S&P Global Manufacturing PMI Final",
   "impact": "Low",
   "forecast": "53.1",
   "previous": "53.9",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T09:00:00.000Z",
   "country": "EUR",
   "title": "CPI Flash",
   "impact": "Low",
   "forecast": "",
   "previous": "103.13",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T09:00:00.000Z",
   "country": "EUR",
   "title": "Core Inflation Rate YoY Flash",
   "impact": "Medium",
   "forecast": "",
   "previous": "2.6%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T09:00:00.000Z",
   "country": "EUR",
   "title": "Inflation Rate YoY Flash",
   "impact": "High",
   "forecast": "",
   "previous": "3.2%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T09:00:00.000Z",
   "country": "EUR",
   "title": "Inflation Rate MoM Flash",
   "impact": "Medium",
   "forecast": "",
   "previous": "0.1%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T09:00:00.000Z",
   "country": "GBP",
   "title": "Index-Linked Gilt 2031 Auction",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T09:30:00.000Z",
   "country": "USD",
   "title": "Challenger Job Cuts",
   "impact": "Low",
   "forecast": "",
   "previous": "97.006",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T11:00:00.000Z",
   "country": "USD",
   "title": "MBA 30-Year Mortgage Rate",
   "impact": "Medium",
   "forecast": "",
   "previous": "6.59",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T11:00:00.000Z",
   "country": "USD",
   "title": "MBA Purchase Index",
   "impact": "Low",
   "forecast": "",
   "previous": "169.7",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T11:00:00.000Z",
   "country": "USD",
   "title": "MBA Mortgage Applications",
   "impact": "Low",
   "forecast": "",
   "previous": "1",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T11:00:00.000Z",
   "country": "USD",
   "title": "MBA Mortgage Refinance Index",
   "impact": "Low",
   "forecast": "",
   "previous": "834.2",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T11:00:00.000Z",
   "country": "USD",
   "title": "MBA Mortgage Market Index",
   "impact": "Low",
   "forecast": "",
   "previous": "272.1",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T12:15:00.000Z",
   "country": "USD",
   "title": "ADP Employment Change",
   "impact": "Medium",
   "forecast": "",
   "previous": "122",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T13:20:00.000Z",
   "country": "USD",
   "title": "NY Fed Bill Purchases 1 to 4 months",
   "impact": "Low",
   "forecast": "6.637",
   "previous": "",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T13:45:00.000Z",
   "country": "USD",
   "title": "S&P Global Manufacturing PMI Final",
   "impact": "Low",
   "forecast": "55.7",
   "previous": "55.1",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T14:00:00.000Z",
   "country": "USD",
   "title": "ISM Manufacturing New Orders",
   "impact": "Low",
   "forecast": "",
   "previous": "56.8",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T14:00:00.000Z",
   "country": "USD",
   "title": "ISM Manufacturing PMI",
   "impact": "High",
   "forecast": "",
   "previous": "54",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T14:00:00.000Z",
   "country": "USD",
   "title": "ISM Manufacturing Prices",
   "impact": "Low",
   "forecast": "",
   "previous": "82.1",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T14:00:00.000Z",
   "country": "USD",
   "title": "ISM Manufacturing Employment",
   "impact": "Medium",
   "forecast": "",
   "previous": "48.6",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T14:00:00.000Z",
   "country": "USD",
   "title": "Construction Spending MoM",
   "impact": "Low",
   "forecast": "",
   "previous": "0.4%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T14:30:00.000Z",
   "country": "USD",
   "title": "EIA Gasoline Production Change",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T14:30:00.000Z",
   "country": "USD",
   "title": "EIA Crude Oil Stocks Change",
   "impact": "Medium",
   "forecast": "",
   "previous": "",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T14:30:00.000Z",
   "country": "USD",
   "title": "EIA Heating Oil Stocks Change",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T14:30:00.000Z",
   "country": "USD",
   "title": "EIA Distillate Fuel Production Change",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T14:30:00.000Z",
   "country": "USD",
   "title": "EIA Crude Oil Imports Change",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T14:30:00.000Z",
   "country": "USD",
   "title": "EIA Gasoline Stocks Change",
   "impact": "Medium",
   "forecast": "",
   "previous": "",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T14:30:00.000Z",
   "country": "USD",
   "title": "EIA Refinery Crude Runs Change",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T14:30:00.000Z",
   "country": "USD",
   "title": "EIA Distillate Stocks Change",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T14:30:00.000Z",
   "country": "USD",
   "title": "EIA Cushing Crude Oil Stocks Change",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T15:30:00.000Z",
   "country": "USD",
   "title": "17-Week Bill Auction",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T22:45:00.000Z",
   "country": "NZD",
   "title": "Building Permits MoM",
   "impact": "Low",
   "forecast": "",
   "previous": "10.9%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T23:50:00.000Z",
   "country": "JPY",
   "title": "Stock Investment by Foreigners",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T23:50:00.000Z",
   "country": "JPY",
   "title": "Foreign Bond Investment",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-01T23:50:00.000Z",
   "country": "JPY",
   "title": "Monetary Base YoY",
   "impact": "Low",
   "forecast": "",
   "previous": "-12.2",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-02T01:30:00.000Z",
   "country": "AUD",
   "title": "Balance of Trade",
   "impact": "High",
   "forecast": "",
   "previous": "1.791 A$",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-02T01:30:00.000Z",
   "country": "AUD",
   "title": "Exports MoM",
   "impact": "Low",
   "forecast": "",
   "previous": "7.2%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-02T01:30:00.000Z",
   "country": "AUD",
   "title": "Imports MoM",
   "impact": "Low",
   "forecast": "",
   "previous": "0.8%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-02T03:35:00.000Z",
   "country": "JPY",
   "title": "10-Year JGB Auction",
   "impact": "Low",
   "forecast": "",
   "previous": "2.649%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-02T06:30:00.000Z",
   "country": "CHF",
   "title": "Inflation Rate YoY",
   "impact": "Medium",
   "forecast": "",
   "previous": "0.6%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-02T06:30:00.000Z",
   "country": "CHF",
   "title": "Inflation Rate MoM",
   "impact": "Low",
   "forecast": "",
   "previous": "0.2%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-02T09:00:00.000Z",
   "country": "EUR",
   "title": "Unemployment Rate",
   "impact": "Medium",
   "forecast": "",
   "previous": "6.3%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-02T09:00:00.000Z",
   "country": "GBP",
   "title": "Green Gilt 2037 Auction",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-02T12:30:00.000Z",
   "country": "USD",
   "title": "Continuing Jobless Claims",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-02T12:30:00.000Z",
   "country": "USD",
   "title": "Jobless Claims 4-week Average",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-02T12:30:00.000Z",
   "country": "USD",
   "title": "Initial Jobless Claims",
   "impact": "Medium",
   "forecast": "",
   "previous": "",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-02T12:30:00.000Z",
   "country": "USD",
   "title": "U-6 Unemployment Rate",
   "impact": "Low",
   "forecast": "",
   "previous": "8.1",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-02T12:30:00.000Z",
   "country": "USD",
   "title": "Manufacturing Payrolls",
   "impact": "Low",
   "forecast": "",
   "previous": "7",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-02T12:30:00.000Z",
   "country": "USD",
   "title": "Non Farm Payrolls",
   "impact": "High",
   "forecast": "",
   "previous": "172",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-02T12:30:00.000Z",
   "country": "USD",
   "title": "Average Hourly Earnings MoM",
   "impact": "Medium",
   "forecast": "",
   "previous": "0.3",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-02T12:30:00.000Z",
   "country": "USD",
   "title": "Unemployment Rate",
   "impact": "High",
   "forecast": "",
   "previous": "4.3",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-02T12:30:00.000Z",
   "country": "USD",
   "title": "Average Hourly Earnings YoY",
   "impact": "Medium",
   "forecast": "",
   "previous": "3.4",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-02T12:30:00.000Z",
   "country": "USD",
   "title": "Nonfarm Payrolls Private",
   "impact": "Low",
   "forecast": "",
   "previous": "120",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-02T12:30:00.000Z",
   "country": "USD",
   "title": "Government Payrolls",
   "impact": "Low",
   "forecast": "",
   "previous": "52",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-02T12:30:00.000Z",
   "country": "USD",
   "title": "Participation Rate",
   "impact": "Medium",
   "forecast": "",
   "previous": "61.8%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-02T12:30:00.000Z",
   "country": "USD",
   "title": "Average Weekly Hours",
   "impact": "Low",
   "forecast": "",
   "previous": "34.3",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-02T13:30:00.000Z",
   "country": "CAD",
   "title": "S&P Global Manufacturing PMI",
   "impact": "Medium",
   "forecast": "",
   "previous": "52.9",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-02T14:00:00.000Z",
   "country": "USD",
   "title": "Factory Orders ex Transportation",
   "impact": "Low",
   "forecast": "",
   "previous": "1.3%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-02T14:00:00.000Z",
   "country": "USD",
   "title": "Factory Orders MoM",
   "impact": "Medium",
   "forecast": "",
   "previous": "4.8%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-02T14:30:00.000Z",
   "country": "USD",
   "title": "EIA Natural Gas Stocks Change",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-02T15:30:00.000Z",
   "country": "USD",
   "title": "4-Week Bill Auction",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-02T15:30:00.000Z",
   "country": "USD",
   "title": "8-Week Bill Auction",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-02T16:00:00.000Z",
   "country": "USD",
   "title": "15-Year Mortgage Rate",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-02T16:00:00.000Z",
   "country": "USD",
   "title": "30-Year Mortgage Rate",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-02T17:00:00.000Z",
   "country": "USD",
   "title": "Baker Hughes Oil Rig Count",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-02T17:00:00.000Z",
   "country": "USD",
   "title": "Baker Hughes Total Rigs Count",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-02T18:00:00.000Z",
   "country": "USD",
   "title": "Total Vehicle Sales",
   "impact": "Low",
   "forecast": "",
   "previous": "16.1",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-02T20:30:00.000Z",
   "country": "USD",
   "title": "Fed Balance Sheet",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-02T22:00:00.000Z",
   "country": "NZD",
   "title": "ANZ Roy Morgan Consumer Confidence",
   "impact": "Low",
   "forecast": "",
   "previous": "86.5",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-02T23:00:00.000Z",
   "country": "AUD",
   "title": "S&P Global Composite PMI Final",
   "impact": "Low",
   "forecast": "49.8",
   "previous": "48.7",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-02T23:00:00.000Z",
   "country": "AUD",
   "title": "S&P Global Services PMI Final",
   "impact": "Low",
   "forecast": "49.9",
   "previous": "48.7",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-03T00:00:00.000Z",
   "country": "USD",
   "title": "Independence Day",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-03T00:30:00.000Z",
   "country": "JPY",
   "title": "S&P Global Services PMI Final",
   "impact": "Low",
   "forecast": "51.8",
   "previous": "50",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-03T00:30:00.000Z",
   "country": "JPY",
   "title": "S&P Global Composite PMI Final",
   "impact": "Low",
   "forecast": "52.5",
   "previous": "51.1",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-03T01:45:00.000Z",
   "country": "CNY",
   "title": "RatingDog Services PMI",
   "impact": "Medium",
   "forecast": "",
   "previous": "54.4",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-03T01:45:00.000Z",
   "country": "CNY",
   "title": "RatingDog Composite PMI",
   "impact": "Low",
   "forecast": "",
   "previous": "54",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-03T03:35:00.000Z",
   "country": "JPY",
   "title": "3-Month Bill Auction",
   "impact": "Low",
   "forecast": "",
   "previous": "",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-03T08:00:00.000Z",
   "country": "EUR",
   "title": "S&P Global Services PMI Final",
   "impact": "Low",
   "forecast": "48.9",
   "previous": "47.7",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-03T08:00:00.000Z",
   "country": "EUR",
   "title": "S&P Global Composite PMI Final",
   "impact": "Low",
   "forecast": "49.5",
   "previous": "48.5",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-03T08:30:00.000Z",
   "country": "GBP",
   "title": "DMP 1Y CPI Expectations",
   "impact": "Low",
   "forecast": "",
   "previous": "3.7%",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-03T08:30:00.000Z",
   "country": "GBP",
   "title": "S&P Global Composite PMI Final",
   "impact": "Low",
   "forecast": "49.4",
   "previous": "49.7",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-03T08:30:00.000Z",
   "country": "GBP",
   "title": "S&P Global Services PMI Final",
   "impact": "Low",
   "forecast": "48.7",
   "previous": "49.3",
   "actual": null,
   "quelle": "tradingview"
  },
  {
   "date": "2026-07-03T08:30:00.000Z",
   "country": "GBP",
   "title": "DMP 3M Output Price Expectations",
   "impact": "Low",
   "forecast": "",
   "previous": "4%",
   "actual": null,
   "quelle": "tradingview"
  }
 ],
 "briefing": {
  "datum": "2026-06-19",
  "typ": "freitag",
  "titel": "Iran-Deal unterschrieben, Öl bei 80$ — aber die Fed dreht hart auf Falke und der Dollar explodiert nach oben",
  "tagesnews": {
   "stand": "2026-06-24",
   "wochentag": "Mittwoch",
   "tenor": "Der Dollar ist König — aber alle sind jetzt long Dollar in den großen US-Inflationswert (PCE) morgen hinein. Genau das macht es zweischneidig: Eine kühle Zahl kann eine schnelle Gegenbewegung auslösen.",
   "heute": [
    "🇦🇺 Australien-Inflation (Mai): Die Schlagzeile kühlte überraschend auf 4,0% y/y ab (erwartet waren 4,4%) — sieht erst mal gut aus. ABER der ehrliche Kern (Trimmed Mean) STIEG auf 3,6% — der höchste Stand seit September 2024 und über der Erwartung (3,5%). Heißt: Die hartnäckige Teuerung ist noch da, die RBA kann nicht so schnell senken. Für den Aussie ein gemischtes Signal — der heiße Kern stützt ihn ein bisschen gegen den starken Dollar.",
    "🇯🇵 Achtung Yen: USD/JPY steht bei rund 161,5 — der Yen ist brutal schwach. Japans Finanzministerium warnt jetzt offen, es sei „jederzeit bereit zu reagieren\". Das ist genau die Sprache, die kurz vor einer echten Intervention (Yen-Stützungskäufe) kommt. Wer auf weiter steigenden USD/JPY setzt, spielt mit dem Feuer — eine Intervention kann das Paar in Minuten mehrere Hundert Pips nach unten reißen.",
    "⏳ Alles wartet auf MORGEN: Der US-PCE (der Lieblings-Inflationswert der Fed) kommt Donnerstag 14:30 Wiener Zeit. Die ganze Dollar-Stärke hängt an dieser einen Zahl. Heiß = Falken-Fed bestätigt, Dollar läuft weiter. Kühl = die harte Fed-Geschichte wackelt und es kann einen kräftigen Dollar-Rückschlag geben."
   ],
   "gestern": [
    "🌡️ Gestern (Di 23.6.) war der große Tag: die Flash-PMIs — das Frühwarn-Thermometer der Wirtschaft (über 50 = Wachstum, unter 50 = Schrumpfen). Ergebnis war ein KRASSER Unterschied USA vs. Europa.",
    "🇺🇸 Die US-Industrie sprang auf 55,7 — der beste Wert seit Mai 2022. Die US-Wirtschaft brummt einfach weiter.",
    "🇪🇺 Europa dagegen schrumpft: Eurozone 49,5 (unter 50), Deutschland 48,0 (schlechtester Wert seit 1,5 Jahren). 🇬🇧 Englands Dienstleister stürzten auf 48,7 — die stärkste Schrumpfung seit Anfang 2023.",
    "💵 Die Folge: Der Dollar schoss auf ein 13-Monats-Hoch (Dollar-Index ~101). „USA stark, Europa schwach\" zieht das Geld in den Dollar. EUR/USD fiel auf rund 1,1375.",
    "🇨🇦 Davor (Mo 22.6.) kam die Kanada-Inflation überraschend HEISS rein: 3,2% y/y (erwartet 3,0%), getrieben vom Benzin (+33% durch die Hormus-Störung). Aber der Kern liegt mit ~2% am Ziel, und die Bank of Canada „schaut bewusst drüber hinweg\" — deshalb half es dem schwachen Loonie kaum."
   ]
  },
  "lage": [
   "Die größte Nachricht zuerst: Der Iran-Deal ist HEUTE (Freitag, 19.6.) in der Schweiz UNTERSCHRIEBEN worden! Damit ist der Öl-Schock vom Frühjahr offiziell vorbei. US-Öl ist auf rund 80$ gefallen (Brent ~83$) — so billig wie zuletzt Anfang März, ganze 20%+ unter dem Kriegs-Hoch. Billiges Öl heißt: Die Inflation wird in den nächsten Monaten von ganz alleine sinken.",
   "ABER — und das war die Überraschung der Woche — die Fed schaut da bewusst drüber hinweg. Am Mittwoch hielt sie die Zinsen wie erwartet bei 3,50–3,75% (einstimmig 12:0 bei Warshs Debüt). Der Hammer kam aber vom 'Dot Plot' (die Zins-Landkarte der Fed-Mitglieder): Der dreht von 'wir senken bald' auf 'wir erhöhen eher noch'. Die mittlere Zinsprognose für Ende 2026 sprang von 3,4% auf 3,8%. Neun Mitglieder sehen jetzt mindestens eine ERHÖHUNG, nur eines noch eine Senkung.",
   "Der Grund: Die Fed hat ihre eigene Inflationsprognose (PCE) für 2026 von 2,7% auf satte 3,6% hochgeschraubt. Sie glaubt also, die Teuerung bleibt zäh — Öl hin oder her. Folge an den Märkten: Der Dollar-Index schoss zurück über die 100er-Marke, EUR/USD fiel rund 60 Pips Richtung 1,15. Der Markt preist jetzt sogar eine mögliche Fed-Erhöhung schon im September ein. Aus 'Dollar verliert seine Story' wurde über Nacht 'Dollar ist König'.",
   "Zentralbank-Superwoche im Schnelldurchlauf: Japan (BOJ) hat am Dienstag wie erwartet auf 1,00% ERHÖHT — höchster Satz seit den 1990ern, und die BOJ signalisiert weitere Schritte. Australien (RBA) hielt bei 4,35%, die Schweiz (SNB) bei 0%, England (BOE) bei 3,75% — dort stimmten aber 2 von 9 Mitgliedern für eine ERHÖHUNG (7:2). Niemand hat gesenkt — die Notenbanken haben mehr Angst vor Inflation als vor schwacher Konjunktur.",
   "England-Inflation (Mittwoch): Die Schlagzeile blieb mit 2,8% y/y überraschend ZAHM (erwartet waren 3,0%), Monatswert nur +0,2%. ABER die Dienstleistungs-Inflation — der Lieblings-Wert der BOE — sprang von 3,2% auf 3,7% hoch. Deshalb auch die zwei Erhöhungs-Stimmen. Gemischtes Bild: Schlagzeile kühl, Kern heiß.",
   "US-Konsum brummt weiter: Die Einzelhandelsumsätze stiegen im Mai um +0,9% (fast doppelt so viel wie erwartet), die wichtige Kern-Gruppe +0,7%. Achtung: Ein Teil kommt von teurem Benzin und auslaufenden Steuerrückzahlungen — der Rückenwind lässt nach. Trotzdem: starker Verbraucher = Argument für die Falken-Fed.",
   "Neuseeland-BIP (Donnerstag): +0,8% zum Vorquartal (knapp unter den erwarteten +0,9%), aber +1,5% im Jahr (deutlich über den erwarteten +1,1%). Gemischt — und unter dem brutalen Dollar-Druck ging der Kiwi unter."
  ],
  "waehrungen": {
   "USD": {
    "stimmung": "bullisch",
    "score": 45,
    "grund": "KOMPLETTE WENDE zur Vorwoche: Die Fed dreht auf Falke (Dot Plot zeigt jetzt Erhöhung statt Senkung, PCE-Prognose auf 3,6% angehoben), der Einzelhandel brummt, der Dollar-Index ist zurück über 100. Der Markt spekuliert sogar auf eine Erhöhung im September. Solange das hält, ist der Dollar das Maß aller Dinge."
   },
   "EUR": {
    "stimmung": "bärisch",
    "score": -25,
    "grund": "Die EZB hat zwar erhöht (2,40%), aber das reicht nicht: Die Fed klingt jetzt noch härter, der Zinsabstand zu den USA wird wieder größer. EUR/USD fiel Richtung 1,15. Solange die Fed den Ton angibt, hat der Euro das Nachsehen."
   },
   "GBP": {
    "stimmung": "bärisch",
    "score": -20,
    "grund": "Gemischt mit Schlagseite nach unten: Schlagzeilen-Inflation kühl (2,8%), aber Dienstleistungen heiß (3,7%) und 2 BOE-Mitglieder wollten erhöhen. Die Wirtschaft schrumpfte zuletzt (-0,1%). Stagflations-Gefahr bleibt — und gegen den starken Dollar verliert das Pfund trotzdem."
   },
   "JPY": {
    "stimmung": "neutral",
    "score": 5,
    "grund": "Strukturell besser: BOJ hat auf 1,00% erhöht und signalisiert weitere Schritte — endlich echter Zins-Rückhalt. ABER kurzfristig 'Sell the Fact' nach der Erhöhung plus der bullige Dollar drücken den Yen wieder. Zins-Plus und Dollar-Stärke heben sich gerade auf."
   },
   "CHF": {
    "stimmung": "bärisch",
    "score": -20,
    "grund": "SNB bleibt bei 0% — null Zinsvorteil. Und mit dem unterschriebenen Iran-Deal schmilzt die Nachfrage nach dem 'sicheren Hafen' Franken. Gegen den starken Dollar doppelt im Nachteil."
   },
   "CAD": {
    "stimmung": "bärisch",
    "score": -22,
    "grund": "Kanada-Inflation kam Montag zwar HEISS rein (3,2% y/y, über Erwartung) — aber nur wegen Benzin; der Kern liegt am 2%-Ziel und die BOC „schaut bewusst drüber hinweg\". Öl-Crash (~80$) + starker Dollar überwiegen klar. Loonie bleibt schwach."
   },
   "AUD": {
    "stimmung": "neutral",
    "score": -8,
    "grund": "Australiens Kern-Inflation (Trimmed Mean) stieg auf 3,6% (höchster seit Sept 2024) — das hält die RBA hart und stützt den Aussie etwas. Gegen den überall starken Dollar zieht er aber noch den Kürzeren. Donnerstag kommt der Australien-Arbeitsmarkt."
   },
   "NZD": {
    "stimmung": "bärisch",
    "score": -25,
    "grund": "Niedriger Zins (2,25%), BIP zum Vorquartal knapp verfehlt, und der starke Dollar walzt alles platt. Der Kiwi gehört zu den Schwächsten — er hat weder Zins- noch Wachstums-Story gegen den Dollar."
   }
  },
  "paare": [
   {
    "paar": "USD/CAD",
    "score": 32,
    "treiber": "Doppelschlag für den Loonie: starker Dollar TRIFFT auf Öl-Crash (~80$ nach Iran-Deal). Kanada-Inflation am Montag der nächste Test. Paar tendiert klar nach oben."
   },
   {
    "paar": "USD/CHF",
    "score": 24,
    "treiber": "Falken-Fed treibt den Dollar, während der Franken null Zins hat und seine Krisen-Prämie mit dem Iran-Deal verliert. Beides schiebt das Paar nach oben."
   },
   {
    "paar": "EUR/USD",
    "score": -28,
    "treiber": "Der Zinsabstand USA–Europa öffnet sich wieder, nachdem die Fed auf Falke gedreht hat. EUR/USD bereits Richtung 1,15 gefallen. Bärisch, solange die Fed den Ton angibt."
   },
   {
    "paar": "NZD/USD",
    "score": -26,
    "treiber": "Schwächste Story der Majors: niedriger NZ-Zins, BIP q/q verfehlt, plus voller Dollar-Druck. Kiwi ohne Gegenwehr."
   },
   {
    "paar": "GBP/USD",
    "score": -24,
    "treiber": "Pfund gemischt (Schlagzeile kühl, Dienstleistungen heiß), aber gegen den starken Dollar überwiegt die Schwäche. Stagflations-Schatten bleibt."
   },
   {
    "paar": "USD/JPY",
    "score": 12,
    "treiber": "Trend bullish (starker Dollar), ABER bei ~161,5 wächst das INTERVENTIONSRISIKO: Japans Finanzministerium warnt offen, „jederzeit bereit\" zu sein. Eine Intervention kann das Paar in Minuten brutal nach unten reißen — deshalb Score gedeckelt. PCE am Donnerstag ist der Joker."
   },
   {
    "paar": "AUD/USD",
    "score": -12,
    "treiber": "Weniger bärisch als zuletzt: Australiens Kern-Inflation (Trimmed Mean) stieg auf 3,6% (höchster seit Sept 2024) — das hält die RBA hart und stützt den Aussie. Gegen den starken Dollar überwiegt aber noch die Schwäche."
   }
  ],
  "wochenausblick": {
   "text": [
    "Nach der Notenbank-Schlacht kommt die ruhigere, aber wichtige Daten-Woche. Das EINE Ereignis, auf das alles zuläuft, ist der US-PCE-Preisindex am Donnerstag (25.6., 14:30 Wien). PCE ist der Lieblings-Inflationswert der Fed — und genau diese Fed hat ihre PCE-Prognose gerade auf 3,6% hochgeschraubt. Bestätigt der echte Wert das, ist die Falken-Story wasserdicht und der Dollar läuft weiter. Kommt er kühler rein, wackelt die ganze Erzählung.",
    "Am Dienstag (23.6.) kommen die 'Flash-PMIs' für Deutschland, die Eurozone, England und die USA. Das sind Stimmungs-Umfragen unter Einkaufsmanagern — eine Art Frühwarn-Thermometer für die Wirtschaft. Über 50 = Wachstum, unter 50 = Schrumpfen. Schwache Europa-PMIs würden den ohnehin angeschlagenen Euro weiter drücken.",
    "Dazwischen: Kanada-Inflation (Mo), China-Leitzinsen (Mo), Australien-Inflation (Mi) und Australien-Arbeitsmarkt (Do). Für AUD und CAD die Heimspiele der Woche — aber alle stehen im Schatten des großen Dollar-Trends.",
    "Großes Bild: Es kämpfen zwei Kräfte. KRAFT 1 — der Iran-Deal drückt das Öl und damit die Inflation (gut gegen den Dollar). KRAFT 2 — die Fed ignoriert das und droht mit Erhöhung (gut für den Dollar). Diese Woche hat KRAFT 2 klar gewonnen. Der PCE am Donnerstag entscheidet, ob das so bleibt."
   ],
   "termine": [
    {
     "tag": "Montag 22.6.",
     "land": "CNY",
     "titel": "China: Leitzinsen (Loan Prime Rate)",
     "warum": "Setzt China weiter auf billiges Geld zum Ankurbeln? Lockerung stützt tendenziell AUD und NZD (China kauft dann mehr Rohstoffe)."
    },
    {
     "tag": "Montag 22.6.",
     "land": "CAD",
     "titel": "Kanada-Inflation (CPI) Mai",
     "warum": "Erster großer Test für den Loonie. Billiges Öl drückt die Teuerung — eine kühle Zahl gibt der BOC Spielraum und schwächt den CAD zusätzlich."
    },
    {
     "tag": "Dienstag 23.6.",
     "land": "EUR",
     "titel": "Flash-PMIs Deutschland + Eurozone (Juni)",
     "warum": "Frühwarn-Thermometer der Wirtschaft. Schwache Werte (unter 50) treffen den ohnehin schwachen Euro hart."
    },
    {
     "tag": "Dienstag 23.6.",
     "land": "GBP",
     "titel": "Flash-PMIs England (Juni)",
     "warum": "Zeigt, ob Englands Wirtschaft weiter schwächelt. Schwach = mehr Stagflations-Sorgen = Pfund unter Druck."
    },
    {
     "tag": "Dienstag 23.6.",
     "land": "USD",
     "titel": "Flash-PMIs USA (Juni)",
     "warum": "Starke US-PMIs würden die Falken-Fed-Story und den starken Dollar weiter füttern."
    },
    {
     "tag": "Mittwoch 24.6.",
     "land": "AUD",
     "titel": "Australien-Inflation (CPI) Mai",
     "warum": "Heiße Zahl = RBA bleibt hart = stützt den AUD. Kühle Zahl = AUD fällt im Dollar-Sog noch tiefer."
    },
    {
     "tag": "Donnerstag 25.6.",
     "land": "USD",
     "titel": "US-PCE-Preisindex (Mai) — 14:30 Wien",
     "warum": "DAS Event der Woche. Lieblings-Inflationswert der Fed. Bestätigt er die hochgeschraubte 3,6%-Erwartung, läuft der Dollar weiter. Kühler = Dollar-Rückschlag."
    },
    {
     "tag": "Donnerstag 25.6.",
     "land": "AUD",
     "titel": "Australien-Arbeitsmarkt (Mai)",
     "warum": "Starker Jobmarkt hält die RBA hart und stützt den AUD. Schwach = noch mehr Druck."
    },
    {
     "tag": "Donnerstag 25.6.",
     "land": "USD",
     "titel": "US-BIP Q1 (finale Schätzung) — 14:30 Wien",
     "warum": "Bestätigt das Wachstumstempo der US-Wirtschaft. Begleitwert zum PCE — selten ein großer Bewegungs-Auslöser, aber im Blick behalten."
    },
    {
     "tag": "Freitag 26.6.",
     "land": "JPY",
     "titel": "Japan-Inflation (Tokio-CPI Juni)",
     "warum": "Frühindikator für Japans Teuerung. Heiß = bestätigt den BOJ-Erhöhungskurs = stützt den Yen."
    }
   ]
  },
  "prognosen": [
   {
    "event": "US-PCE-Preisindex Mai (USD)",
    "termin": "Donnerstag 25.6., 14:30 Wiener Zeit",
    "prognoseMarkt": "Erwartet wird ein leichter Anstieg der Kern-PCE-Jahresrate; die Fed selbst hat ihre 2026-PCE-Prognose gerade auf 3,6% angehoben",
    "meineEinschaetzung": "Ich erwarte einen Wert, der ZÄH bleibt — also keine Entwarnung bei der Kern-Teuerung. Begründung: Die Fed hat diese Woche nicht ohne Grund auf Falke gedreht; sie sieht in den eigenen Daten klebrige Inflation (Dienstleistungen, Wohnen). Der Öl-Rückgang vom Iran-Deal wirkt erst mit Verzögerung und schlägt zuerst auf die Schlagzeile, nicht auf den Kern. ABER ich bin hier ehrlich unsicher: Wenn der Mai-PCE überraschend kühl reinkommt, kollidiert das sofort mit der harten Fed-Botschaft — dann gibt es einen heftigen Dollar-Rückschlag, weil der Markt die September-Erhöhung wieder auspreist.",
    "wennHoeher": "Kern-PCE heiß / über Erwartung: Die Falken-Fed ist bestätigt, der Dollar läuft weiter. EUR/USD unter 1,15, USD/CAD und USD/CHF steigen weiter, Gold unter Druck.",
    "wennNiedriger": "Kern-PCE kühl / unter Erwartung: Widerspruch zur harten Fed → kräftiger Dollar-Rückschlag. EUR/USD erholt sich über 1,15, Gold und risikoreiche Währungen (AUD, NZD) springen nach oben. Genau das Szenario, in dem der Iran-Deal-Effekt endlich durchschlägt."
   },
   {
    "event": "Flash-PMIs Eurozone Juni (EUR)",
    "termin": "Dienstag 23.6., ca. 10:00 Wiener Zeit",
    "prognoseMarkt": "Erwartet werden Werte nahe der 50er-Schwelle (Stagnation), Dienstleistung etwas über, Industrie etwas unter 50",
    "meineEinschaetzung": "Ich erwarte durchwachsene bis leicht schwache Werte. Begründung: Der Öl-Rückgang hilft Europas Industrie zwar mittelfristig, aber im Juni war die Lage noch von Krieg und Unsicherheit geprägt. Wichtig fürs Trading: Selbst neutrale PMIs treffen den Euro derzeit härter als sonst, weil er ohnehin im Dollar-Sog steht.",
    "wennHoeher": "PMIs deutlich über 50: Euro bekommt etwas Boden zurück, EUR/USD-Erholung möglich — aber gegen den starken Dollar nur ein Strohfeuer, wenn der PCE nicht mitspielt.",
    "wennNiedriger": "PMIs unter 50 (Schrumpfen): Euro fällt weiter, EUR/USD Richtung 1,14. Bestätigt das Bild 'Europa schwach, USA stark'."
   },
   {
    "event": "Kanada-Inflation Mai (CAD)",
    "termin": "Montag 22.6., 14:30 Wiener Zeit",
    "prognoseMarkt": "Erwartet wird eine moderate, eher nachlassende Teuerung dank fallender Energiepreise",
    "meineEinschaetzung": "Ich tendiere zu einer eher KÜHLEN Zahl. Begründung: Der Ölpreis ist nach dem Iran-Deal abgestürzt, und Energie ist ein großer Treiber der kanadischen Inflation. Das gibt der BOC Argumente, locker zu bleiben — schlecht für den CAD, der ohnehin unter dem Öl-Crash leidet.",
    "wennHoeher": "Inflation heißer als erwartet: CAD bekommt kurz Unterstützung (BOC könnte länger pausieren), USD/CAD-Anstieg wird gebremst.",
    "wennNiedriger": "Inflation kühl: Doppel-Belastung für den Loonie (billiges Öl + lockere BOC). USD/CAD läuft weiter nach oben."
   },
   {
    "event": "Australien-Inflation Mai (AUD)",
    "termin": "Mittwoch 24.6., ca. 03:30 Wiener Zeit",
    "prognoseMarkt": "Erwartet wird eine immer noch erhöhte, aber langsam nachlassende Teuerung",
    "meineEinschaetzung": "Ich erwarte eine noch hartnäckige Zahl. Begründung: Die RBA hat gerade NICHT gesenkt und klingt hart — sie sieht die Inflation noch nicht besiegt. Eine heiße CPI-Zahl wäre die beste Stütze für den AUD, der sonst im Dollar-Sog untergeht.",
    "wennHoeher": "CPI heiß: RBA-Härte bestätigt, AUD/USD-Erholungsversuch — aber gegen den starken Dollar begrenzt.",
    "wennNiedriger": "CPI kühl: Markt rechnet wieder mit RBA-Senkungen, AUD fällt im Dollar-Sog weiter."
   }
  ],
  "lehren": [
   "GROSSE LEHRE DER WOCHE: Ein bekanntes 'Halten' kann trotzdem ein Erdbeben sein — wenn die PROGNOSEN (Dot Plot) sich drehen. Die Fed hielt erwartungsgemäß, aber der Schwenk des Dot Plots von Senkung auf Erhöhung und die auf 3,6% angehobene PCE-Prognose haben den Dollar regelrecht explodieren lassen (DXY über 100, EUR/USD ~60 Pips runter). Beim nächsten Notenbank-Termin nicht nur auf die Zinszahl schauen, sondern auf die mitgelieferten Prognosen — da steckt die echte Bewegung.",
   "MEINE PROGNOSEN diese Woche — ehrliche Bilanz: BOJ-Erhöhung auf 1,00% TRAF ✓, Fed-Halten mit hawkishem Dot Plot TRAF sogar sehr genau ✓✓ (genau so begründet), RBA-Halten ✓, SNB-Halten ✓, BOE-Halten mit hawkishen Dissens-Stimmen ✓ (2:9 für Erhöhung — ich hatte aufs Abstimmungsverhältnis hingewiesen). DANEBEN lag ich bei UK-CPI: Ich erwartete 'über 3%', es kamen kühle 2,8% — ABER die Dienstleistungs-Inflation sprang auf 3,7%, der heiße Kern, vor dem ich gewarnt hatte, war also doch da. Lehre: Schlagzeilen-Inflation und Kern können auseinanderlaufen — bei UK immer auf die Dienstleistungen schauen, das ist der BOE-Wert.",
   "WICHTIGSTE FX-LEHRE: Meine Paar-Scores der Vorwoche lagen großteils FALSCH, weil ich den Dollar zu schwach eingeschätzt hatte. Ich hatte EUR/USD bullish (+28) und USD/JPY bärisch (-32) — tatsächlich rauschte der Dollar durch die Falken-Fed nach oben, EUR/USD fiel und USD/JPY stieg (BOJ-Erhöhung wurde von 'Sell the Fact' + Dollar-Stärke überlagert). Lehre: Eine Zentralbank-Überraschung (hier die Fed) kann ALLE Paare gleichzeitig in eine Richtung reißen und Einzel-Stories (BOJ-Erhöhung) übertönen. Wenn die Fed tagt, ist der Dollar-Trend wichtiger als jede einzelne Gegenwährung. Deshalb sind diese Woche fast alle meine Scores pro-Dollar ausgerichtet.",
   "Der Iran-Deal-Effekt ist real (Öl ~80$, unterschrieben), aber er wirkt LANGSAM und über die Inflation — nicht sofort über die Notenbanken. Die Fed schaut bewusst drüber hinweg. Lehre: Ein disinflationärer Schock (billiges Öl) schlägt sich nicht am selben Tag in der Geldpolitik nieder. Der echte Test ist der PCE nächste Woche: Erst wenn die Inflations-DATEN fallen, kann die Fed weicher werden. Bis dahin gewinnt die harte Notenbank-Rhetorik gegen die gute Öl-Nachricht.",
   "TECHNISCHE LEHRE FÜRS SYSTEM: Die kostenlosen Feeds (ForexFactory + TradingView) waren diese Woche dauerhaft mit HTTP 403 blockiert — deshalb stehen die Ist-Werte der Superwoche (BOJ, Fed, RBA, SNB, BOE, UK-CPI, US-Retail, NZ-BIP) NICHT in historie.json, sondern nur hier im Briefing-Text. Auch die automatische Treffer-Quote-Auswertung konnte nicht laufen (frankfurter-Zugriff im Cloud-Lauf ebenfalls blockiert). TODO für die nächste lokale Session: Sobald die Feeds wieder gehen, diese Ist-Werte manuell via setze-ist-wert.mjs nachtragen, damit das Momentum-Lernen weiterläuft."
  ]
 },
 "lexikon": {
  "hinweis": "Lexikon für das Dashboard. 'muster' ist ein Regex, der auf den Event-Titel von ForexFactory passt. Erklärungen bewusst kinderleicht.",
  "begriffe": [
   {
    "muster": "Core CPI|Core Inflation Rate",
    "name": "Kern-Inflation (Core CPI)",
    "einfach": "Misst, wie viel teurer die Dinge geworden sind – ABER ohne Essen und Benzin, weil deren Preise wild hin- und herspringen. Das ist die 'ehrliche' Inflation, auf die die Zentralbank am meisten schaut.",
    "warum": "Die Zentralbank entscheidet anhand dieser Zahl, ob sie die Zinsen anhebt oder senkt. Höhere Zinsen = stärkere Währung.",
    "hoeher": "Währung wird meist STÄRKER (Zentralbank muss eher die Zinsen hochhalten/anheben).",
    "niedriger": "Währung wird meist SCHWÄCHER (Zinssenkungen werden wahrscheinlicher)."
   },
   {
    "muster": "CPI(?!.*Core)|Consumer Price|Prelim CPI|Final CPI|HICP|Tokyo Core|Inflation Rate",
    "name": "Inflation (Verbraucherpreise, CPI)",
    "einfach": "Der große Einkaufskorb-Test: Was kostet das Leben (Essen, Benzin, Miete...) im Vergleich zu früher? Steigt die Zahl, wird alles teurer.",
    "warum": "Zu hohe Inflation zwingt die Zentralbank, die Zinsen anzuheben. Das macht die Währung attraktiver.",
    "hoeher": "Währung meist STÄRKER (mehr Druck für hohe Zinsen).",
    "niedriger": "Währung meist SCHWÄCHER (Tür für Zinssenkungen geht auf)."
   },
   {
    "muster": "PPI|Producer Price",
    "name": "Erzeugerpreise (PPI)",
    "einfach": "Was Fabriken für ihre Waren verlangen, BEVOR sie im Laden landen. Wie ein Frühwarnsystem: Wird es hier teurer, wird es bald auch für uns alle teurer.",
    "warum": "Läuft der Inflation oft ein paar Monate voraus – Händler reichen höhere Kosten an Kunden weiter.",
    "hoeher": "Währung tendenziell STÄRKER (künftige Inflation → Zinsen bleiben hoch).",
    "niedriger": "Währung tendenziell SCHWÄCHER."
   },
   {
    "muster": "Core PCE|PCE Price",
    "name": "PCE-Inflation (Liebling der Fed)",
    "einfach": "Noch ein Inflationsmesser – aber DER, den die US-Zentralbank (Fed) offiziell als Zielgröße benutzt. Ziel: 2%.",
    "warum": "Wenn die Fed über Zinsen redet, meint sie diese Zahl. Weit über 2% = Zinsen bleiben hoch.",
    "hoeher": "USD meist STÄRKER.",
    "niedriger": "USD meist SCHWÄCHER."
   },
   {
    "muster": "Non-Farm|NFP",
    "name": "US-Arbeitsmarktbericht (NFP)",
    "einfach": "Wie viele neue Jobs in den USA im letzten Monat entstanden sind (ohne Bauernhöfe). DIE wichtigste Zahl des Monats – jeden ersten Freitag. Die Märkte springen dabei oft wie verrückt.",
    "warum": "Viele neue Jobs = starke Wirtschaft = Zinsen können hoch bleiben. Wenige Jobs = Wirtschaft schwächelt = Zinssenkungen rücken näher.",
    "hoeher": "USD meist STÄRKER (und oft fallen Gold & Aktien kurz).",
    "niedriger": "USD meist SCHWÄCHER."
   },
   {
    "muster": "Unemployment Rate",
    "name": "Arbeitslosenquote",
    "einfach": "Wie viel Prozent der Leute, die arbeiten wollen, keinen Job finden. Niedrig = Wirtschaft brummt.",
    "warum": "Steigende Arbeitslosigkeit ist oft das erste Zeichen, dass die Wirtschaft kippt – dann senken Zentralbanken die Zinsen.",
    "hoeher": "Währung meist SCHWÄCHER (Achtung: hier ist 'höher' schlecht!).",
    "niedriger": "Währung meist STÄRKER."
   },
   {
    "muster": "Unemployment Claims|Jobless Claims",
    "name": "US-Erstanträge auf Arbeitslosenhilfe",
    "einfach": "Wie viele Amerikaner sich DIESE WOCHE frisch arbeitslos gemeldet haben. Kommt jeden Donnerstag – wie ein wöchentlicher Fiebermesser für den Jobmarkt.",
    "warum": "Schnellster Frühindikator überhaupt: Steigen die Anträge mehrere Wochen, kriselt es.",
    "hoeher": "USD meist SCHWÄCHER (mehr Arbeitslose = schlecht).",
    "niedriger": "USD meist STÄRKER."
   },
   {
    "muster": "Employment Change|Payrolls(?!.*Non)",
    "name": "Beschäftigungsänderung",
    "einfach": "Wie viele Jobs im Land dazugekommen oder weggefallen sind.",
    "warum": "Mehr Jobs = mehr Konsum = stärkere Wirtschaft = Argument für höhere Zinsen.",
    "hoeher": "Währung meist STÄRKER.",
    "niedriger": "Währung meist SCHWÄCHER."
   },
   {
    "muster": "Average (Hourly )?Earnings|Wage Price|Average Cash Earnings|Labor Cost",
    "name": "Löhne / Stundenlöhne",
    "einfach": "Wie stark die Gehälter gestiegen sind. Mehr Lohn klingt gut – aber wenn ALLE mehr verdienen, heben Geschäfte die Preise an → Inflation.",
    "warum": "Zentralbanken fürchten die 'Lohn-Preis-Spirale'. Starke Löhne = Zinsen bleiben länger hoch.",
    "hoeher": "Währung meist STÄRKER.",
    "niedriger": "Währung meist SCHWÄCHER."
   },
   {
    "muster": "Federal Funds Rate|FOMC Statement|FOMC Press|FOMC Economic Projections|Fed Interest Rate|Fed Press Conference",
    "name": "US-Zinsentscheid (Fed/FOMC)",
    "einfach": "Die US-Zentralbank legt den wichtigsten Zins der Welt fest. Stell dir vor: der Preis, den Banken fürs Geldleihen zahlen. Alle 6 Wochen – das größte Event am Finanzmarkt.",
    "warum": "Höhere Zinsen machen den Dollar attraktiver (mehr Zinsen fürs Sparen). Schon ein einzelnes WORT im Statement kann die Märkte drehen.",
    "hoeher": "USD STÄRKER bei Anhebung oder 'harter' Wortwahl (hawkish).",
    "niedriger": "USD SCHWÄCHER bei Senkung oder 'weicher' Wortwahl (dovish)."
   },
   {
    "muster": "Main Refinancing Rate|ECB Press|Monetary Policy Statement|ECB Interest Rate",
    "name": "EZB-Zinsentscheid",
    "einfach": "Die Europäische Zentralbank legt den Zins für die Eurozone fest. Wie bei der Fed – nur für den Euro.",
    "warum": "Bestimmt direkt, wie attraktiv der Euro ist. Die Pressekonferenz (45 Min. später) bewegt oft mehr als die Entscheidung selbst.",
    "hoeher": "EUR STÄRKER bei Anhebung/harten Tönen.",
    "niedriger": "EUR SCHWÄCHER bei Senkung/weichen Tönen."
   },
   {
    "muster": "Official Bank Rate|MPC.*Vote|BOE|BoE Interest Rate|Monetary Policy Summary",
    "name": "Zinsentscheid Bank of England",
    "einfach": "Großbritanniens Zentralbank legt den Zins fürs Pfund fest. Spannend: Man sieht, wie die 9 Mitglieder abgestimmt haben (z.B. 7-2).",
    "warum": "Das Abstimmungsergebnis verrät, wohin die Reise geht – mehr Abweichler = Richtungswechsel naht.",
    "hoeher": "GBP STÄRKER.",
    "niedriger": "GBP SCHWÄCHER."
   },
   {
    "muster": "Overnight Rate|BOC|BoC Interest Rate",
    "name": "Zinsentscheid Bank of Canada",
    "einfach": "Kanadas Zentralbank legt den Zins für den kanadischen Dollar fest.",
    "warum": "Kanada hängt stark am Öl und an den USA – die BOC reagiert oft früher als die Fed.",
    "hoeher": "CAD STÄRKER.",
    "niedriger": "CAD SCHWÄCHER."
   },
   {
    "muster": "Cash Rate|RBA",
    "name": "Zinsentscheid Australien (RBA)",
    "einfach": "Australiens Zentralbank legt den Zins für den Aussie-Dollar fest.",
    "warum": "Australien lebt vom Rohstoff-Export nach China – RBA-Entscheidungen spiegeln oft Chinas Gesundheit.",
    "hoeher": "AUD STÄRKER.",
    "niedriger": "AUD SCHWÄCHER."
   },
   {
    "muster": "RBNZ|NZ Interest",
    "name": "Zinsentscheid Neuseeland (RBNZ)",
    "einfach": "Neuseelands Zentralbank legt den Zins für den Kiwi-Dollar fest.",
    "warum": "Die RBNZ ist oft die mutigste – sie bewegt sich gern vor allen anderen.",
    "hoeher": "NZD STÄRKER.",
    "niedriger": "NZD SCHWÄCHER."
   },
   {
    "muster": "SNB|Libor Rate|Policy Rate.*CHF",
    "name": "Zinsentscheid Schweiz (SNB)",
    "einfach": "Die Schweizer Nationalbank legt den Zins für den Franken fest. Sie überrascht gern ohne Vorwarnung!",
    "warum": "Der Franken ist eine 'sichere Hafen'-Währung – die SNB kämpft oft dagegen, dass er ZU stark wird.",
    "hoeher": "CHF STÄRKER.",
    "niedriger": "CHF SCHWÄCHER."
   },
   {
    "muster": "BOJ|Bank of Japan",
    "name": "Zinsentscheid Japan (BOJ)",
    "einfach": "Japans Zentralbank. Jahrzehntelang waren die Zinsen bei null – jede kleine Änderung ist deshalb ein Erdbeben für den Yen.",
    "warum": "Viele Anleger leihen sich billige Yen, um anderswo zu investieren ('Carry Trade'). Hebt die BOJ die Zinsen, wird das rückabgewickelt → Yen schießt hoch.",
    "hoeher": "JPY STÄRKER.",
    "niedriger": "JPY SCHWÄCHER."
   },
   {
    "muster": "GDP|Gross Domestic",
    "name": "Wirtschaftswachstum (BIP/GDP)",
    "einfach": "Das Zeugnis der ganzen Wirtschaft: Alles, was ein Land produziert und verkauft, zusammengezählt. Wächst die Zahl, geht's dem Land gut.",
    "warum": "Schrumpft die Wirtschaft zwei Quartale nacheinander, heißt das 'Rezession' – dann senken Zentralbanken die Zinsen.",
    "hoeher": "Währung meist STÄRKER.",
    "niedriger": "Währung meist SCHWÄCHER."
   },
   {
    "muster": "Flash Manufacturing PMI|Manufacturing PMI|ISM Manufacturing",
    "name": "Industrie-Stimmung (PMI)",
    "einfach": "Eine Umfrage unter Fabrik-Chefs: 'Läuft's gerade gut oder schlecht?' Über 50 = gut, unter 50 = schlecht. Ganz einfach.",
    "warum": "Fabrik-Chefs merken zuerst, wenn Aufträge wegbrechen – Monate bevor es offizielle Zahlen zeigen.",
    "hoeher": "Währung meist STÄRKER (vor allem über 50).",
    "niedriger": "Währung meist SCHWÄCHER (vor allem unter 50)."
   },
   {
    "muster": "Flash Services PMI|Services PMI|ISM (Non-Manufacturing|Services)",
    "name": "Dienstleistungs-Stimmung (PMI)",
    "einfach": "Gleiche Umfrage, aber bei Restaurants, Banken, Friseuren & Co. Über 50 = gut, unter 50 = schlecht. Wichtiger als Industrie, weil Dienstleistungen den Großteil der Wirtschaft ausmachen.",
    "warum": "Zeigt, ob die Leute noch Geld ausgeben. Die Preise-Unterkomponente ist ein Inflations-Frühwarner.",
    "hoeher": "Währung meist STÄRKER.",
    "niedriger": "Währung meist SCHWÄCHER."
   },
   {
    "muster": "Retail Sales",
    "name": "Einzelhandelsumsätze",
    "einfach": "Wie viel die Leute im Laden und online eingekauft haben. Shoppen die Leute fleißig, brummt die Wirtschaft.",
    "warum": "Konsum ist der Motor der Wirtschaft (in den USA ~70%!). Kein Konsum = Motor stottert.",
    "hoeher": "Währung meist STÄRKER.",
    "niedriger": "Währung meist SCHWÄCHER."
   },
   {
    "muster": "Trade Balance|Balance of Trade",
    "name": "Handelsbilanz",
    "einfach": "Verkauft ein Land mehr ins Ausland als es einkauft (Überschuss) oder umgekehrt (Defizit)?",
    "warum": "Wer viel exportiert, dessen Währung wird gebraucht (Ausländer müssen sie kaufen) → tendenziell stärker.",
    "hoeher": "Währung tendenziell STÄRKER (mehr Überschuss).",
    "niedriger": "Währung tendenziell SCHWÄCHER."
   },
   {
    "muster": "Consumer Confidence|Consumer Sentiment|Westpac Consumer",
    "name": "Verbraucher-Stimmung",
    "einfach": "Eine Umfrage: 'Wie geht's dir finanziell, und traust du dich, Geld auszugeben?' Gute Laune = die Leute kaufen mehr.",
    "warum": "Ängstliche Verbraucher sparen statt shoppen → Wirtschaft kühlt ab. Bei der Uni-Michigan-Umfrage sind auch die INFLATIONSERWARTUNGEN wichtig.",
    "hoeher": "Währung meist STÄRKER.",
    "niedriger": "Währung meist SCHWÄCHER."
   },
   {
    "muster": "Ifo Business|German Ifo",
    "name": "Ifo-Geschäftsklima (Deutschland)",
    "einfach": "DIE Stimmungsumfrage unter 9.000 deutschen Firmenchefs. Deutschlands wichtigster Frühindikator.",
    "warum": "Deutschland ist die größte Wirtschaft der Eurozone – hustet das Ifo, bekommt der Euro Schnupfen.",
    "hoeher": "EUR meist STÄRKER.",
    "niedriger": "EUR meist SCHWÄCHER."
   },
   {
    "muster": "ZEW",
    "name": "ZEW-Konjunkturerwartungen",
    "einfach": "Umfrage unter Finanzprofis: 'Wird die deutsche Wirtschaft in 6 Monaten besser oder schlechter dastehen?'",
    "warum": "Blickt nach vorn statt zurück – Profis riechen Trends früh.",
    "hoeher": "EUR meist STÄRKER.",
    "niedriger": "EUR meist SCHWÄCHER."
   },
   {
    "muster": "JOLTS",
    "name": "Offene Stellen USA (JOLTS)",
    "einfach": "Wie viele offene Jobs es in den USA gibt. Viele offene Stellen = Firmen suchen verzweifelt Leute = Löhne steigen.",
    "warum": "Die Fed schaut hierauf, um zu sehen, ob der Jobmarkt 'überhitzt' ist.",
    "hoeher": "USD meist STÄRKER.",
    "niedriger": "USD meist SCHWÄCHER."
   },
   {
    "muster": "ADP",
    "name": "ADP-Jobbericht (privater Vorbote)",
    "einfach": "Eine private Firma (Lohnabrechner ADP) zählt 2 Tage vor dem offiziellen NFP-Bericht die neuen Jobs. Wie eine Generalprobe.",
    "warum": "Gibt einen Vorgeschmack auf den großen NFP-Freitag – liegt aber oft daneben!",
    "hoeher": "USD meist STÄRKER.",
    "niedriger": "USD meist SCHWÄCHER."
   },
   {
    "muster": "Durable Goods",
    "name": "Langlebige Güter (Bestellungen)",
    "einfach": "Bestellungen für teure Dinge, die lange halten: Autos, Flugzeuge, Maschinen. Solche Käufe macht man nur, wenn man optimistisch ist.",
    "warum": "Firmen investieren nur in Maschinen, wenn sie an die Zukunft glauben.",
    "hoeher": "USD meist STÄRKER.",
    "niedriger": "USD meist SCHWÄCHER."
   },
   {
    "muster": "Building Permits|Housing Starts|Existing Home|New Home|HPI",
    "name": "Häusermarkt",
    "einfach": "Wie viele Häuser gebaut, genehmigt oder verkauft werden. Der Häusermarkt reagiert als ERSTER auf Zinsänderungen (Stichwort Hypothek).",
    "warum": "Hohe Zinsen → teure Kredite → keiner baut/kauft → Frühwarnsignal für die ganze Wirtschaft.",
    "hoeher": "Währung meist STÄRKER.",
    "niedriger": "Währung meist SCHWÄCHER."
   },
   {
    "muster": "Empire State|Philly Fed|Richmond",
    "name": "Regionale US-Industrie-Umfragen",
    "einfach": "Mini-Stimmungstests bei Fabriken in einzelnen US-Regionen (New York, Philadelphia...). Erscheinen vor den großen Zahlen.",
    "warum": "Frühe Puzzleteile für das große Bild. Einzeln schwach, zusammen aussagekräftig.",
    "hoeher": "USD leicht STÄRKER.",
    "niedriger": "USD leicht SCHWÄCHER."
   },
   {
    "muster": "Industrial Production|Factory Orders",
    "name": "Industrieproduktion / Auftragseingänge",
    "einfach": "Wie viel die Fabriken tatsächlich hergestellt haben bzw. wie viele neue Aufträge reingekommen sind.",
    "warum": "Harte Fakten statt Umfragen – zeigt, ob die Industrie wirklich liefert.",
    "hoeher": "Währung meist STÄRKER.",
    "niedriger": "Währung meist SCHWÄCHER."
   },
   {
    "muster": "Crude Oil Inventories",
    "name": "US-Öllagerbestände",
    "einfach": "Wie viel Öl die USA auf Lager haben. Mehr Öl im Lager = weniger Nachfrage = Ölpreis fällt (und umgekehrt).",
    "warum": "Bewegt den Ölpreis → und damit den kanadischen Dollar (Öl-Land!) und die Inflationserwartungen.",
    "hoeher": "Ölpreis runter → CAD schwächer.",
    "niedriger": "Ölpreis rauf → CAD stärker."
   },
   {
    "muster": "OPEC",
    "name": "OPEC-Treffen",
    "einfach": "Die größten Öl-Länder treffen sich und entscheiden: Pumpen wir mehr oder weniger Öl? Weniger Öl = höherer Preis.",
    "warum": "Ölpreis = Inflationstreiber Nr. 1. Betrifft CAD direkt, und über die Inflation alle Währungen.",
    "hoeher": "Förderkürzung → Öl rauf → CAD stärker, Inflationssorgen rauf.",
    "niedriger": "Mehr Förderung → Öl runter → CAD schwächer."
   },
   {
    "muster": "Speaks|Speech|Testimony|Testifies",
    "name": "Rede eines Zentralbankers",
    "einfach": "Ein wichtiger Zentralbanker hält eine Rede. Die Märkte hängen an jedem Wort: Klingt er 'hart' (hawkish = für hohe Zinsen) oder 'weich' (dovish = für Zinssenkungen)?",
    "warum": "Zentralbanker nutzen Reden, um Märkte vorzubereiten. Ein einziger Satz kann mehr bewegen als eine ganze Datenveröffentlichung.",
    "hoeher": "Hawkish (hart) → Währung stärker.",
    "niedriger": "Dovish (weich) → Währung schwächer."
   },
   {
    "muster": "Minutes|Meeting Minutes",
    "name": "Sitzungsprotokoll der Zentralbank",
    "einfach": "Das Protokoll der letzten Zinssitzung wird veröffentlicht – man liest nach, WORÜBER gestritten wurde und wie einig man sich war.",
    "warum": "Verrät, ob die nächste Zinsänderung näher rückt als gedacht.",
    "hoeher": "Hawkish Töne → Währung stärker.",
    "niedriger": "Dovish Töne → Währung schwächer."
   },
   {
    "muster": "Tankan",
    "name": "Tankan-Bericht (Japan)",
    "einfach": "Japans große Firmen-Umfrage (von der Zentralbank selbst!): Wie läuft's bei den großen Herstellern?",
    "warum": "Die BOJ nutzt ihren eigenen Bericht für Zinsentscheidungen.",
    "hoeher": "JPY meist STÄRKER.",
    "niedriger": "JPY meist SCHWÄCHER."
   },
   {
    "muster": "Caixin",
    "name": "Caixin-PMI (China)",
    "einfach": "Stimmungstest bei Chinas kleineren Privatfirmen. Über 50 = läuft, unter 50 = läuft nicht.",
    "warum": "China kauft die Rohstoffe der Welt – geht's China schlecht, leiden AUD und NZD zuerst.",
    "hoeher": "AUD/NZD meist STÄRKER.",
    "niedriger": "AUD/NZD meist SCHWÄCHER."
   },
   {
    "muster": "Current Account",
    "name": "Leistungsbilanz",
    "einfach": "Die große Gesamtrechnung eines Landes mit dem Ausland: Waren, Dienstleistungen, Zinsen, Überweisungen – alles zusammen.",
    "warum": "Dauerhafte Defizite machen eine Währung langfristig anfällig.",
    "hoeher": "Währung tendenziell STÄRKER.",
    "niedriger": "Währung tendenziell SCHWÄCHER."
   },
   {
    "muster": "Bank Holiday",
    "name": "Bankfeiertag",
    "einfach": "Banken in dem Land haben zu. Weniger Händler am Markt = entweder Totentanz oder wilde Sprünge bei dünnem Handel.",
    "warum": "Vorsicht: Bei wenig Liquidität können kleine Orders große Bewegungen auslösen.",
    "hoeher": "—",
    "niedriger": "—"
   }
  ],
  "fallback": {
   "name": "Wirtschaftsdaten",
   "einfach": "Eine Wirtschaftszahl aus diesem Land. Grundregel: Besser als die Prognose = gut für die Währung. Schlechter = schlecht für die Währung.",
   "warum": "Märkte vergleichen IMMER mit der Prognose, nicht mit der Vergangenheit. Die Überraschung zählt.",
   "hoeher": "Meist Währung stärker (wenn 'mehr' hier 'besser' bedeutet).",
   "niedriger": "Meist Währung schwächer."
  },
  "waehrungen": {
   "USD": {
    "flagge": "🇺🇸",
    "name": "US-Dollar",
    "spitzname": "Greenback",
    "zentralbank": "Fed"
   },
   "EUR": {
    "flagge": "🇪🇺",
    "name": "Euro",
    "spitzname": "Single Currency",
    "zentralbank": "EZB"
   },
   "GBP": {
    "flagge": "🇬🇧",
    "name": "Britisches Pfund",
    "spitzname": "Cable",
    "zentralbank": "BoE"
   },
   "JPY": {
    "flagge": "🇯🇵",
    "name": "Japanischer Yen",
    "spitzname": "Sicherer Hafen",
    "zentralbank": "BoJ"
   },
   "CHF": {
    "flagge": "🇨🇭",
    "name": "Schweizer Franken",
    "spitzname": "Swissy",
    "zentralbank": "SNB"
   },
   "CAD": {
    "flagge": "🇨🇦",
    "name": "Kanadischer Dollar",
    "spitzname": "Loonie (Öl-Währung)",
    "zentralbank": "BoC"
   },
   "AUD": {
    "flagge": "🇦🇺",
    "name": "Australischer Dollar",
    "spitzname": "Aussie (Rohstoff-Währung)",
    "zentralbank": "RBA"
   },
   "NZD": {
    "flagge": "🇳🇿",
    "name": "Neuseeland-Dollar",
    "spitzname": "Kiwi",
    "zentralbank": "RBNZ"
   },
   "CNY": {
    "flagge": "🇨🇳",
    "name": "Chinesischer Yuan",
    "spitzname": "Renminbi",
    "zentralbank": "PBoC"
   },
   "All": {
    "flagge": "🌍",
    "name": "Global",
    "spitzname": "",
    "zentralbank": ""
   }
  }
 },
 "leitzinsen": {
  "hinweis": "Leitzinsen der G10-Zentralbanken. Wird in Claude-Sessions/Briefings gepflegt. satz = Zahl in % (für Differenz-Berechnung), anzeige = Text, richtung = rauf|runter|halten (Erwartung nächste Sitzung).",
  "stand": "2026-06-24",
  "zinsen": [
   {
    "code": "AUD",
    "bank": "RBA",
    "satz": 4.35,
    "anzeige": "4,35%",
    "naechste": "Anf. Aug 2026",
    "erwartung": "Halten — am 16.6. bei 4,35% geblieben, Ton blieb hart",
    "richtung": "halten"
   },
   {
    "code": "GBP",
    "bank": "BOE",
    "satz": 3.75,
    "anzeige": "3,75%",
    "naechste": "Anf. Aug 2026",
    "erwartung": "Halten — am 18.6. 7:2 geblieben (2 wollten ERHÖHEN)",
    "richtung": "halten"
   },
   {
    "code": "USD",
    "bank": "Fed",
    "satz": 3.625,
    "anzeige": "3,50–3,75%",
    "naechste": "29.7.2026",
    "erwartung": "Halten, aber Dot Plot zeigt jetzt ERHÖHUNG statt Senkung — Markt preist mögliche Erhöhung im Sept",
    "richtung": "halten"
   },
   {
    "code": "NZD",
    "bank": "RBNZ",
    "satz": 2.25,
    "anzeige": "2,25%",
    "naechste": "8.7.2026",
    "erwartung": "Halten bei 2,25% (nach 6 Senkungen jetzt Pause)",
    "richtung": "halten"
   },
   {
    "code": "EUR",
    "bank": "EZB",
    "satz": 2.4,
    "anzeige": "2,40%",
    "naechste": "23.7.2026",
    "erwartung": "Halten nach frischer Erhöhung im Juni",
    "richtung": "halten"
   },
   {
    "code": "CAD",
    "bank": "BOC",
    "satz": 2.25,
    "anzeige": "2,25%",
    "naechste": "29.7.2026",
    "erwartung": "Halten — billiges Öl drückt die Inflation",
    "richtung": "halten"
   },
   {
    "code": "JPY",
    "bank": "BOJ",
    "satz": 1,
    "anzeige": "1,00%",
    "naechste": "Ende Jul 2026",
    "erwartung": "Pause nach Juni-Erhöhung — weitere Erhöhung 2026 signalisiert",
    "richtung": "halten"
   },
   {
    "code": "CHF",
    "bank": "SNB",
    "satz": 0,
    "anzeige": "0,00%",
    "naechste": "24.9.2026",
    "erwartung": "Halten bei 0% — am 18.6. bestätigt",
    "richtung": "halten"
   }
  ]
 },
 "momentum": {
  "USD": {
   "ueber": 2,
   "unter": 1,
   "gleich": 3,
   "gesamt": 6,
   "score": 1
  },
  "CAD": {
   "ueber": 0,
   "unter": 1,
   "gleich": 3,
   "gesamt": 4,
   "score": -1
  },
  "EUR": {
   "ueber": 0,
   "unter": 0,
   "gleich": 1,
   "gesamt": 1,
   "score": 0
  },
  "GBP": {
   "ueber": 0,
   "unter": 0,
   "gleich": 1,
   "gesamt": 1,
   "score": 0
  },
  "AUD": {
   "ueber": 1,
   "unter": 2,
   "gleich": 0,
   "gesamt": 3,
   "score": -1
  }
 },
 "prognoseQuote": {
  "wochenAusgewertet": 1,
  "treffer": 1,
  "gesamt": 7,
  "quote": 14,
  "wochenErfasst": 2,
  "letzte": [
   {
    "woche": "2026-06-15",
    "quote": 14,
    "treffer": 1,
    "gesamt": 7
   }
  ]
 },
 "historie": [
  {
   "datum": "2026-06-23T21:30:00-04:00",
   "land": "AUD",
   "titel": "CPI m/m",
   "impact": "High",
   "prognose": "-0.4%",
   "vorher": "0.4%",
   "actual": "-0.7%",
   "notiz": null
  },
  {
   "datum": "2026-06-23T21:30:00-04:00",
   "land": "AUD",
   "titel": "CPI y/y",
   "impact": "High",
   "prognose": "4.3%",
   "vorher": "4.2%",
   "actual": "4.0%",
   "notiz": null
  },
  {
   "datum": "2026-06-23T21:30:00-04:00",
   "land": "AUD",
   "titel": "Trimmed Mean CPI m/m",
   "impact": "High",
   "prognose": "0.3%",
   "vorher": "0.3%",
   "actual": "0.5%",
   "notiz": null
  },
  {
   "datum": "2026-06-22T08:30:00-04:00",
   "land": "CAD",
   "titel": "CPI m/m",
   "impact": "High",
   "prognose": "0.7%",
   "vorher": "0.4%",
   "actual": "0.5%",
   "notiz": null
  },
  {
   "datum": "2026-06-22T08:30:00-04:00",
   "land": "CAD",
   "titel": "Median CPI y/y",
   "impact": "High",
   "prognose": "2.1%",
   "vorher": "2.1%",
   "actual": "2.1%",
   "notiz": null
  },
  {
   "datum": "2026-06-22T08:30:00-04:00",
   "land": "CAD",
   "titel": "Trimmed CPI y/y",
   "impact": "High",
   "prognose": "2.0%",
   "vorher": "2.0%",
   "actual": "2.0%",
   "notiz": null
  },
  {
   "datum": "2026-06-12T02:00:00-04:00",
   "land": "GBP",
   "titel": "GDP m/m",
   "impact": "High",
   "prognose": "-0.1%",
   "vorher": "0.3%",
   "actual": "-0.1%",
   "notiz": null
  },
  {
   "datum": "2026-06-11T08:30:00-04:00",
   "land": "USD",
   "titel": "Core PPI m/m",
   "impact": "High",
   "prognose": "0.5%",
   "vorher": "1.0%",
   "actual": "0.8%",
   "notiz": null
  },
  {
   "datum": "2026-06-11T08:30:00-04:00",
   "land": "USD",
   "titel": "PPI m/m",
   "impact": "High",
   "prognose": "0.7%",
   "vorher": "1.4%",
   "actual": "1.1%",
   "notiz": null
  },
  {
   "datum": "2026-06-11T08:15:00-04:00",
   "land": "EUR",
   "titel": "Main Refinancing Rate",
   "impact": "High",
   "prognose": "2.40%",
   "vorher": "2.15%",
   "actual": "2.40%",
   "notiz": null
  },
  {
   "datum": "2026-06-10T09:45:00-04:00",
   "land": "CAD",
   "titel": "Overnight Rate",
   "impact": "High",
   "prognose": "2.25%",
   "vorher": "2.25%",
   "actual": "2.25%",
   "notiz": null
  },
  {
   "datum": "2026-06-10T08:30:00-04:00",
   "land": "USD",
   "titel": "Core CPI m/m",
   "impact": "High",
   "prognose": "0.3%",
   "vorher": "0.4%",
   "actual": "0.2%",
   "notiz": null
  },
  {
   "datum": "2026-06-10T08:30:00-04:00",
   "land": "USD",
   "titel": "Core CPI y/y",
   "impact": "High",
   "prognose": "2.9%",
   "vorher": "2.8%",
   "actual": "2.9%",
   "notiz": null
  },
  {
   "datum": "2026-06-10T08:30:00-04:00",
   "land": "USD",
   "titel": "CPI m/m",
   "impact": "High",
   "prognose": "0.5%",
   "vorher": "0.6%",
   "actual": "0.5%",
   "notiz": null
  },
  {
   "datum": "2026-06-10T08:30:00-04:00",
   "land": "USD",
   "titel": "CPI y/y",
   "impact": "High",
   "prognose": "4.2%",
   "vorher": "3.8%",
   "actual": "4.2%",
   "notiz": null
  }
 ],
 "anzahlGespeichert": 163
};
